...
| **FF5** | Toda tabela com dado de usuário tem RLS por `auth.uid()` — **EMENDADA em 2026-09-11 (migração 0022)**: existe **uma** exceção declarada, a leitura cruzada aluno→personal sob vínculo aceito. Ela é `for select` apenas e vive em policies SEPARADAS das de `auth.uid()`, nunca com `or` dentro delas. Consequência que a emenda obriga, e que custou 12 correções no mesmo dia: **RLS deixou de significar "só o meu", então toda consulta de leitura filtra `usuario_id` explicitamente** | Query no catálogo do Postgres; tabelas sem policy = 0. Mais dois checks nascidos da emenda: nenhuma policy de escrita (`insert`/`update`/`delete`) menciona `vinculo_personal`; e nenhum `select` de `treino`/`serie`/`usuario` em `src/` sem filtro de dono — varrido à mão em 11/set, 12 pontos corrigidos, sendo o pior deles a Home do personal listando os treinos do aluno |
