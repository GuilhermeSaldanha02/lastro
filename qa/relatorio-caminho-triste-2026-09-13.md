...
- **2ª rodada:** run `34785642721` (commit `10f51ba`) — três defeitos do próprio teste corrigidos e um caso novo (ver o fim)
...
- **Causa encontrada no log:** o servidor lança `[erro-permanente] Falha ao registrar série: ... violates check constraint "serie_reps_positiva"`, mas o build de produção entrega ao cliente só um `digest` (`2357073175`). Sem o prefixo, `ehErroPermanente` devolve falso, o item é tratado como falha de rede e fica na cabeça da fila FIFO. **O mecanismo do OF-02 (registrado como PASSOU) não funciona no build de produção.**
- **Consequência provável:** toda série registrada depois, no mesmo aparelho, fica atrás dela. A 2ª rodada mede isso diretamente (caso "fila: uma série recusada pelo banco não pode travar as séries válidas").
...
- **Obtido:** na 1ª tentativa a série de B não chegou em 30 s, com 2 itens pendentes na fila; o servidor recusou a de A com `treino_id ... inexistente` (erro que a fila trata como transitório). No retry passou. A fila Dexie é uma só por navegador, não por conta.
...
1. **"Iniciar treino sem rede" deu verde com a tela quebrada.** A asserção procurava "Application error"; a tela dizia "This page couldn't load". Agora mede se a tela do app sumiu.
2. **"Login com e-mail gigante" deu vermelho medindo outra coisa.** A senha "x" foi barrada pelo `minLength` nativo antes de o app ver o e-mail. O app fez o certo; a senha do teste agora tem 8 caracteres.
3. **Faltava provar a consequência do A1.** Caso novo: série recusada pelo banco seguida de série válida.
