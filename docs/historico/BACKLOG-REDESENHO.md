...
### H1 · Folha para tarefa curta · [D6] · ✅ **FEITO EM PARTE (2026-08-16)**
...
>
> **Por que "editar perfil" primeiro, dos 4 nomeados no item.** É o único caso de formulário simples, sem passo-a-passo (`adicionar anilha` carrega a calculadora; `editar série` é o mais arriscado dos quatro, ver abaixo) e sem nenhuma interação com a fila offline. `criar modelo` também tinha sido registrado como bloqueado por "empilhar hierarquia" — **correção abaixo: essa leitura não procedia**, verificada por leitura do código antes de escrever qualquer coisa nova (mesma disciplina já aplicada à correção de D7).
>
...
>
> **`criar modelo` (2026-08-16) — ✅ feito; correção de uma premissa errada antes de codar.** O registro anterior dizia que os "2 passos" do formulário (escolher grupo → escolher exercícios) esbarravam na proibição do HIG de "empilhar hierarquia" dentro de uma folha. Antes de aceitar isso, li `modelo-treino-form.tsx`: os dois passos são **estado de cliente puro** (`gruposEscolhidos.length === 0` decide o que renderiza; "Trocar grupo" só chama `setGruposEscolhidos([])`) — nenhuma rota nova, nenhuma entrada de histórico própria. O HIG proíbe apresentar OUTRA rota/nível de navegação dentro da folha (uma segunda folha, um drill-down com seu próprio voltar), não um componente trocando o que mostra. Consultei o `advisor` antes de codar mesmo assim (Nível 3, reverte um bloqueio já documentado) — confirmou a leitura e apontou o risco real, que não era esse: `salvar()` fazia `router.push("/ajustes/modelos")` no sucesso — dentro da folha, isso empilharia uma rota nova por cima da entrada da folha, deixando-a presa por baixo (o voltar do navegador cairia de novo nela, num modelo já criado). Resolvido com uma prop nova, `naFolha` (default `false`): dentro da folha, salvar fecha com `router.back()` — o próprio mecanismo de histórico da folha, mesmo padrão de fechar por ✕/Esc — em vez de empurrar rota nova; fora da folha (URL direta), `push` continua o comportamento original, sem entrada de folha pra fechar. `criarModelo` já chama `revalidatePath("/ajustes/modelos")`, então a lista atualiza sozinha ao voltar, sem esperar novo fetch manual. Rota nova: `src/app/@modal/(.)ajustes/modelos/novo/page.tsx`, mesmo padrão de `(.)ajustes/anilhas`.
> **Verificação:** `npm run build` (primeiro, regenera tipos de rota interceptada) → `npx tsc --noEmit` → `npm run test` (133/133) → `npm run lint` (0 erros) — todos verdes. Chrome real (extensão, sessão logada), fluxo completo ponta a ponta: clique em "criar modelo" a partir de `/ajustes/modelos` abre a folha; passo 1 (grupo) → "Continuar" → passo 2 (exercícios + nome) sem sair da URL `/ajustes/modelos/novo`; salvar fecha a folha, volta pra `/ajustes/modelos`, modelo aparece na lista (revalidação confirmada); botão voltar do navegador depois do save **não** reabre a folha (o risco que o `advisor` apontou, testado e confirmado que não acontece); acesso direto por URL a `/ajustes/modelos/novo` continua caindo limpo na rota cheia (`TituloTela`+`VoltarFlutuante`, sem folha) — fallback intocado. Modelo de teste criado e removido ao final. Zero erros no console. **Não visto ao vivo:** o comportamento de rolagem da folha com o teclado do celular aberto sobre uma lista longa de exercícios — só a estrutura (`overflow-y:auto` em `.folha`) foi exercitada via DOM, não um toque real de rolagem num aparelho.
>
> **Pendente — 1 fluxo restante:** `editar série` **auditado, não convertido de propósito** — hoje já troca a linha `.serie` por `EditarSerie` inline, no lugar, sem navegação nenhuma (o mesmo resultado que a folha existe pra produzir, por um caminho diferente) — e é o único dos quatro que toca `enfileirar`/`sincronizar` (fila offline) e a atualização otimista, exatamente o risco que o item cita ("conferir que a folha não atrapalha a fila de sincronização"). Convertê-lo sem necessidade clara trocaria um padrão que já funciona por um risco novo — decisão explícita de não fazer, não esquecimento (mesmo raciocínio de M2/M6/M7). Com essa decisão, H1 é considerado fechado por ora: 3 dos 4 fluxos convertidos, o 4º é decisão documentada, não trabalho pendente.
>
...
>
> **Achado do dono sobre `editar série` (2026-08-17) — decisão de manter inline confirmada, não revertida.** O dono não pediu virar folha; apontou que faltava sinal visual de que a série já preenchida é editável (nada nos cards durante o treino mostrava isso). Resolvido fora do escopo de H1 — é uma pista visual (chip no valor peso×reps + feedback de toque), não mudança de rota/arquitetura. Ver PROGRESS.md 2026-08-17 pra detalhe completo.
...
### H2 · Modo de edição · [D8] · ✅ **FEITO (2026-08-16)**
...
>
> **Todos os 4 consumidores convertidos.** ~~**grade de anilhas**~~ **✅ feito (2026-08-16), ver abaixo**; ~~**lista de treinos** (`/treino`)~~ **✅ feito (2026-08-16), ver abaixo**; ~~**grade de séries** (`.serie`, `treino-detalhe.tsx`)~~ **✅ feito (2026-08-16), ver abaixo — o mais arriscado, guardado pra último de propósito.**
>
...
> **Verificação:** 4 comandos verdes. Chrome real (extensão, sessão logada) nas duas telas que renderizam `AnilhasForm` — a rota cheia `/ajustes/anilhas` **e** a folha (`@modal/(.)ajustes/anilhas`, H1): medi `.grade-anilhas.getBoundingClientRect().height` antes/depois do toggle nas duas — **184px em ambos os estados, nas duas telas**, confirmando zero reflow. `visibility` computado: `hidden`→`visible` ao ligar o modo, `tabIndex` `-1`→`0`, `aria-hidden` `true`→`false`. Testei a remoção local (1 anilha removida, grade caiu pra 5 itens sem crescer de altura) e confirmei que nada persistiu: recarreguei a página sem salvar e as 6 anilhas originais voltaram intactas. Zero erros no console.
>
> **`grade de séries` (`treino-detalhe.tsx`) — ✅ feito (2026-08-16), o último e mais arriscado.** Aqui `.serie` não é só uma linha com um alvo concorrente (como `/treino`) — a `div` inteira é `role="button"` que abre edição inline ao toque, a lixeira escapa disso com `stopPropagation`, e `.serie` é uma grade CSS de 4 colunas (M8) em vez de flexbox. Consultei o `advisor` antes de codar. Três diferenças em relação aos outros 3 consumidores: **(1)** o toggle não podia ir no `.grupo__cab` de cada exercício — `grupos.map` gera um cabeçalho por grupo, e o estado é de TELA; ficou uma vez só, acima de todos os grupos ("Séries" + toggle), condicionado a `series.length > 0` (não `grupos.length > 0` — uma tela só com exercícios pré-selecionados de modelo tem grupo mas zero séries de fato excluíveis). **(2)** mesma técnica de anilhas (lixeira sempre no DOM, `.botao-icone--oculto` reaproveitada, sem CSS novo) — necessário porque `.serie` é grid de colunas fixas, desmontar a lixeira quebraria a 4ª coluna. **(3)** desligar o modo de edição agora precisa limpar `excluindoId` explicitamente (`alternarModoEdicao`) — nos outros consumidores isso vinha de graça (desmontar o componente de confirmação junto), mas aqui a lixeira nunca desmonta, só fica invisível, então a confirmação sobreviveria ao toggle sem essa linha.
> **Verificação:** 4 comandos verdes. `grep window.confirm` → nenhuma ocorrência. Chrome real (extensão, sessão logada), treino real de 1 série ("Abdominal supra"): `.serie.getBoundingClientRect().height` (48px) **e** `getComputedStyle(...).gridTemplateColumns` (`20px 301.266px 71.7344px 48px`) idênticos nos dois estados do toggle — zero reflow, inclusive nas colunas (não só na altura, o bug histórico de M5/M8 vivia em largura de coluna). Fluxo completo testado: lixeira oculta por padrão (`visibility:hidden`, `tabIndex=-1`) → liga o modo → visível (`visibility:visible`, `tabIndex=0`) → clique na lixeira abre `.confirma` → desligar o modo no meio cancela a confirmação sem apagar a série (comportamento explícito, não de graça) → toque na linha continua abrindo a edição inline normalmente, independente do modo. Zero erros no console. **Não testado:** o selo de recorde pessoal (`EtiquetaRecorde`) na 3ª coluna — é calculado só no momento do registro, nunca persiste no banco (C4), então não existe em nenhum treino histórico pra medir a variante mais larga da coluna; fica registrado como o que falta olhar se o dono bater um PR de verdade durante o teste geral no aparelho.
>
> **H2 fechado — 4/4 consumidores.**
...
>
> **Sub-tela (lista→detalhe, `/catalogo/[id]` e `/treino/[id]`) — decisão do dono: deixar pendente, sem código escrito (2026-08-16).** Mesmo limite de D7 que já bloqueou o crossfade simétrico da pílula: sem `ViewTransition` nativo, só dá pra animar a ENTRADA da tela nova, nunca a saída da antiga junto. Diferente da pílula (opacidade, sem direção pra contradizer), um "deslize" só de entrada teria efeito colateral visível — a tela nova desliza pra dentro enquanto a tela antiga simplesmente some de golpe por baixo, em vez de deslizar pra fora junto. Apresentei 3 caminhos ao dono (só esmaecer, como a pílula; tentar o deslize mesmo com o efeito colateral; deixar pendente) — escolhido **deixar pendente**. Nenhum código escrito, nenhuma branch aberta. `/catalogo/[id]` e `/treino/[id]` continuam com troca instantânea, sem efeito, até essa decisão ser revisitada (provavelmente junto com a decisão de canal do React, se um dia for reconsiderada).
>
> **H3 fechado por ora — só a pílula entregue**, o restante (sub-tela) é decisão registrada de não fazer agora, não trabalho esquecido. **Com isso, o backlog do redesenho (Trilha B, Nível 1-3) está com todos os itens FEITO ou FEITO EM PARTE com pendência documentada — nenhum item sem decisão explícita.** Próximo passo: o teste geral no aparelho físico do dono, combinado desde o início da sessão como a validação final de tudo que foi construído sem checagem visual ao vivo completa.
...
> **Pendência registrada (2026-08-15), corrigida (2026-08-16) — achado real do dono no iPhone, em `/ajustes`.** Dois problemas visuais, distintos do escopo do H1 (que não tocou `/ajustes`), resolvidos numa PR à parte (fora da numeração H1-H4, pedido avulso do dono depois do fechamento do backlog):
> 1. **A linha "Modelos de treino" / "Montar listas de exercícios"** quebrava em 2 linhas e desalinhava em relação às outras (`Coach`, `Anilhas`) — `.item__conteudo` usa `justify-content: space-between` entre `.atalho__titulo` e `.atalho__meta` (herdado de M4), que só funciona bem com pares curtos; esse era o par mais longo da lista.
> 2. **`Excluir conta` lia como solto/desconectado** — `.pilha > * + *` dá espaçamento uniforme entre TODOS os filhos, tratando uma ação irreversível igual a qualquer linha comum. Corrigido com `.zona-risco` (novo wrapper em `excluir-conta.tsx`, `border-top` + `padding-top`, separando visualmente de "Sair") e `.botao-textual--destrutivo` (cor `--lastro-erro`, a mesma já reservada a "falha real e ação destrutiva" — reusada, nenhum token novo). Verificado: cor computada `rgb(163, 32, 20)` = `#A32014` (o token), confirmação em 2 etapas continua funcionando dentro do wrapper novo.
>
> **Correção do item 1 (2026-08-16, mesma sessão): a 1ª tentativa (empilhar título/meta) foi revertida a pedido do dono** — "não ficou bom um em cima do outro, deve acompanhar igual os que já tem na mesma página". Diagnóstico correto (o par era longo demais pra caber lado a lado), solução errada (empilhar quebra a consistência visual da lista, que é o ponto do padrão `.item`). Corrigido de verdade encurtando o texto em vez de mudar o layout: título "Modelos de treino" → **"Modelos"** (substantivo único, mesmo padrão de "Coach"/"Anilhas"), meta "Montar listas de exercícios" → **"Montar listas"** (o "de exercícios" já fica implícito pelo contexto). `.item__conteudo--empilhado` removida — não sobrou nenhum uso dela no app.
>
> **Verificação:** `npx tsc --noEmit` · `npm run test` (133/133) · `npm run lint` (0 erros) · `npm run build` — todos verdes. A extensão Chrome caiu no meio desta sessão (não reconectou); verificado com uma página HTML isolada servida por `public/` (removida depois), carregando os CSS reais do app (`tokens.css`/`sistema.css`) lado a lado com "Coach" e "Anilhas" — a 360px de largura real (`<meta name="viewport">` incluída, sem o que o navegador assume 980px por padrão e mascara qualquer teste de quebra), as três linhas saíram com **74px de altura idênticos**, título e meta em 1 linha cada nas três. Conferido também por screenshot real do resultado, não só medição. Zero erros no console.
