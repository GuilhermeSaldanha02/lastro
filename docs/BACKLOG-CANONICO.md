...
| PU-06 | MÉDIO | **APROVADO PELO DONO 2026-09-26, sem advogado (decisão dele)**: aceite no fim do texto (`/aceite`, portão em `casca.ts`), políticas em Ajustes (`/ajustes/politicas`). **Falta só o dono dar nome do responsável e e-mail de contato** (`RESPONSAVEL_NOME`/`RESPONSAVEL_CONTATO` em `lib/legal/documentos.ts`). Era: | Termos de Uso e Política de Privacidade (LGPD — o app guarda dado de saúde/treino). Documento jurídico, não só tela — considerar revisão de um advogado antes de publicar. |
