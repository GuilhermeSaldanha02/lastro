...
> 1. **A linha "Modelos de treino" / "Montar listas de exercícios"** quebrava em 2 linhas e desalinhava em relação às outras (`Coach`, `Anilhas`) — `.item__conteudo` usa `justify-content: space-between` entre `.atalho__titulo` e `.atalho__meta` (herdado de M4), que só funciona bem com pares curtos; esse é o par mais longo da lista. Corrigido com um modificador só nessa linha — `.item__conteudo--empilhado` (`flex-direction: column`) — em vez de mudar a classe base, que é reusada por `ListaModelos`/`ListaTreinos`/`/catalogo` e quebraria pares curtos que já funcionam lado a lado. Verificado: título e meta viram 1 linha cada (sem quebra no meio da palavra); `Anilhas`/`Coach` continuam pixel-idênticos a antes (51,6px de altura, inalterados).
...
>
> **Verificação:** `npx tsc --noEmit` · `npm run test` (133/133) · `npm run lint` (0 erros) · `npm run build` — todos verdes. Chrome real (extensão): medição de DOM confirmando os dois pontos acima; zero erros no console. **Limite da sessão de novo:** a aba de teste ficou instável (`document.visibilityState: hidden`, viewport 0×0 em alguns momentos) — precisei fechar e recriar a aba no meio da verificação pra recuperar um viewport real (500px, não os 360-390 pretendidos de iPhone). A medição que discrimina o bug (título/meta virarem 1 linha cada, sem quebra) não depende da largura exata — `flex-direction: column` empilha em qualquer largura — mas o dono ainda deveria confirmar no aparelho real antes de considerar 100% resolvido.
