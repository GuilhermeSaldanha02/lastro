...
| TR-05 | treino | Foco/divisão do sticker pós-treino usa o `grupo_muscular_primario` real do catálogo, não mais um chute por palavra-chave no nome do exercício | ALEGADO (achado do dono ao vivo, dogfooding: treino de pernas — "Afundo com halteres" — saiu rotulado incorretamente. Verificado ao vivo pelo implementador com usuário QA descartável e o mesmo exercício: sticker mostrou "PERNAS" corretamente; `npx vitest run` 238/238, sem regressão. Correção pequena e isolada — dono decidirá se pede auditoria independente antes de mergear, dado a urgência) | (branch `fix/foco-sticker-grupo-real`, ainda não mergeada) | 2026-09-01 | qa/evidencias/TR-05/ |
