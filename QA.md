...
| PE-07 | personal | O estado que ocupa o lugar da prescrição na `/analise` (§11.4.2): sob vínculo o destaque vai para outra pergunta, a prescrição não aparece como card nenhum, e o rodapé diz de quem ela é | **ALEGADO** — gate fechado pelo dono na direção "Troca de posto". Medido no **CSS real** (`tokens.css` + `sistema.css` inlineados num harness, marcação idêntica à do componente): 4 cards e nenhum deles é a pergunta 5; rodapé a 16px/1,55 com a divisória de `--lastro-linha`. **Contraste medido nos 7 temas**, com o pior caso no único tema claro (`branco-ouro`): texto **7,07:1**, nome do personal **6,34:1** — piso AA 4,5. O ouro sobre tema claro é exatamente o par que já reprovou aqui antes (1,11:1 no `.pergunta--primaria`), e desta vez passa porque `--lastro-ouro` é redefinido por tema. **O que isto NÃO prova:** o app rodando. Sem `.env.local` não houve dev server nem conta vinculada — a fiação (`page.tsx` → `vinculo` → componente) está coberta só por `tsc` e pelos 10 testes de `perguntasDaTela` | 3b37fbc | 2026-09-11 | harness em `scratchpad/harness-analise.html` |
