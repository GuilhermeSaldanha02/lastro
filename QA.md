...
| OF-02 | offline | Fila de sincronização (outbox) não trava para sempre quando um item é permanentemente inválido, não só sem rede | PASSOU | PENDENTE-SHA | 2026-08-28 | qa/evidencias/OF-02/ |
| VS-03 | visual | Cabeçalho fixo do treino não sobrepõe conteúdo interativo em viewport curto (teclado aberto) | REPROVOU — `timer-topo.tsx:146-148` mede `contentRect.height` (content-box) no `ResizeObserver`, perdendo o padding vertical (~17px) que a medição inicial (border-box) tinha certo; clearance fica curta em ambos os estados (parado e descanso ativo). Overlap real confirmado com scroll nativo (~5px) e prova causal A/B (injetar o valor correto destrava o clique). Ver `qa/evidencias/VS-03/auditoria-independente/resultado.md` | PENDENTE-SHA | 2026-08-28 | qa/evidencias/VS-03/ |
| TR-02 | treino | Recarregar a tela do treino reflete o estado real do banco logo após uma série sincronizar | PASSOU | PENDENTE-SHA | 2026-08-28 | qa/evidencias/TR-02/ |
| TR-03 | treino | Fluxo completo: criar modelo com 2+ exercícios → iniciar treino a partir dele → registrar série de cada exercício, todos com atalho de plano visível até o fim | PASSOU | PENDENTE-SHA | 2026-08-28 | qa/evidencias/TR-03/ |
| VS-04 | visual | Avatar do cabeçalho mostra a foto/inicial real do usuário em toda página, nunca o placeholder cravado | PASSOU | PENDENTE-SHA | 2026-08-28 | qa/evidencias/VS-04/ |
