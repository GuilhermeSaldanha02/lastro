...
| OF-04 | offline | M1 MÉDIA · Série pendente de uma conta não trava a fila da conta seguinte no mesmo navegador. Obtido: 1ª tentativa travou (2 pendentes, `treino_id inexistente` tratado como transitório); retry passou. Intermitente | REPROVOU | 475c774 | 2026-09-13 | run `34784135645`; qa/evidencias/OF-04/ |
...
| OF-05 | offline | M3 MÉDIA · "Iniciar treino de hoje" sem rede não derruba a tela. Obtido: página de erro do Next em inglês ("This page couldn't load"). Duplo clique com rede cria um treino só (passou) | REPROVOU | 475c774 | 2026-09-13 | run `34784135645`; qa/evidencias/OF-05/ |
...
| TR-10 | treino | B2 BAIXA · Id que não é UUID em `/treino/abc`, `/catalogo/abc`, `/ajustes/relatorios/parecer/abc` e `/api/parecer/abc/pdf` responde 404. Obtido: 500 (`invalid input syntax for type uuid`). UUID inexistente: 404 (passou) | REPROVOU | 475c774 | 2026-09-13 | run `34784135645`; qa/evidencias/log-ci-run-34784135645.txt |
...
| IS-01 | treino | Isolamento entre duas contas sem relação, pela API com o próprio JWT: nenhuma leitura, alteração, exclusão ou inserção em nome da outra em treino, serie, parecer, modelo_treino, modelo_treino_exercicio, uso_ia, vinculo_personal, usuario e storage de avatar; ninguém move série para treino alheio (nem o personal vinculado); `uso_ia` não se apaga; treino, parecer e PDF de outra conta não abrem pelo id; `?modelo=` alheio não traz plano; CSV e progressão do personal só com os dados dele; conta excluída com vínculo vivo não derruba nenhum dos lados | PASSOU | 475c774 | 2026-09-13 | run `34784135645` (`j12`) |
| AU-02 | auth | Sem sessão: 5 APIs → 401 e rotas privadas → login com `?proximo=`; `?proximo=` externo não sai do app; voltar depois de sair ou de excluir a conta não mostra dado; senha de conta excluída não entra; sessão expirada no coach e na Análise avisa, e meta e modelo não fingem ter salvo | PASSOU | 475c774 | 2026-09-13 | run `34784135645` (`j12`, `j13`); qa/evidencias/TR-PUB-01/ |
| AN-05 | analise | 409 com geração em andamento e 429 no teto da análise e do coach, sem gastar cota; 3 pedidos simultâneos ao coach com 9 usos não passam de 10 | PASSOU | 475c774 | 2026-09-13 | run `34784135645` (`j12`) |
| OF-07 | offline | Criar, editar e excluir série sem rede chega ao banco exatamente assim quando a rede volta, sem item descartado | PASSOU | 475c774 | 2026-09-13 | run `34784135645` (`j10`) |
| TR-11 | treino | Reps vazio, negativo, zero, com letras e peso negativo recusados na tela; dois toques rápidos em "Repetir série" criam uma série só | PASSOU | 475c774 | 2026-09-13 | run `34784135645` (`j10`) |
| AJ-05 | ajustes | Modelo: nome só de espaços trava, nome com símbolo e HTML aparece literal, duplo clique cria um. Anilhas: barra 0, negativa e absurda não salvam. Meta 0, 8, 3,5, -1 recusadas na tela e 99 no banco | PASSOU | 475c774 | 2026-09-13 | run `34784135645` (`j11`) |
| VS-08 | visual | Tema corrompido no localStorage não executa script nem quebra a tela (CI e produção) | PASSOU | 475c774 | 2026-09-13 | run `34784135645` (`j13`); qa/evidencias/TR-PUB-01/ |
