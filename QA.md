...
| VS-03 | visual | Cabeçalho fixo do treino não sobrepõe conteúdo interativo em viewport curto (teclado aberto) | REPROVOU — `timer-topo.tsx:146-148` mede `contentRect.height` (content-box) no `ResizeObserver`, perdendo o padding vertical (~17px) que a medição inicial (border-box) tinha certo; clearance fica curta em ambos os estados (parado e descanso ativo). Overlap real confirmado com scroll nativo (~5px) e prova causal A/B (injetar o valor correto destrava o clique). Ver `qa/evidencias/VS-03/auditoria-independente/resultado.md` | c334034 | 2026-08-28 | qa/evidencias/VS-03/ |
