...
- **Última sessão:** 2026-09-13 · agente: claude · branch `fix/coach-teto-atomico`, **empilhada: #245 (QA) → #246 → … → #253 → #254 (índice do M5) → esta (coach concorrente)**. Sem merge; merge na ordem da pilha.
- **O que foi feito:** o dono mandou corrigir o coach concorrente. **Migração `0028_consumir_uso_ia` APLICADA EM PRODUÇÃO**: função que conta e grava o uso de IA numa transação só, com advisory lock por conta e origem (`security invoker`, sem execução para `anon`). `consumirUso` em `src/lib/dados/uso-ia.ts`; `POST /api/coach` lê o vínculo e depois reserva — sem vaga, 429. Decisão em `DECISIONS.md` "2026-09-13 (11)".
- **Migrações aplicadas em produção nesta sessão, a pedido do dono:** `0027_parecer_uma_geracao_por_usuario` (M5) e `0028_consumir_uso_ia` (coach). As duas são só de acréscimo; o código da `main` não chama a 0028.
- **CI da pilha (último de cada):** #252 `34798173235`, #253 `34798655468` e #254 `34799072988` — todos 109 verdes e 1 vermelho, o coach concorrente, que esta branch corrige. Prints do B6 conferidos (lista de modelos e `/perfil` quebram o texto dentro da largura).
- **Verificado localmente:** vitest 47 arquivos e 509 testes (contagem crua; 4 casos novos de `consumirUso`), ESLint e `tsc` limpos. Função conferida em `pg_proc` (invoker, `search_path=public`, `anon` sem execução, `authenticated` com execução).
- **Esperado no CI desta branch:** tudo verde, incluindo "coach: pergunta invisível é recusada e pedidos simultâneos não furam o teto de 10" e "análise e coach: em andamento responde 409 e teto responde 429, sem gastar cota".
- **Em andamento:** o CI deste PR.
- **Não commitado:** nada.
- **Bloqueado / a decidir — tudo do dono:** (1) merge da pilha, na ordem #245 → #255; (2) auditoria independente das correções — `QA.md` mantém os itens corrigidos como REPROVOU, e AN-05 precisa ir para REPROVOU com a evidência do #251; (3) as 5 contas QA antigas; (4) o que já estava: cápsula TREINO / TRABALHO, PE-06, P1/P2.
- **Próximo passo:** ler o CI deste PR; tudo verde = pilha pronta para o dono revisar.
