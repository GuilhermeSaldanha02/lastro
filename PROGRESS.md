...
- **Última sessão:** 2026-09-14 · agente: codex · branch: `feat/consistencia-linguagem-ajuda`.
- **O que foi feito:** triagem read-only da inconsistência de linguagem e da adoção parcial de `DicaInfo`; escopo aprovado para aluno/treino e personal/trabalho, com caminho normal e caminho triste nas três línguas. Desenho registrado em `docs/superpowers/specs/2026-09-14-consistencia-linguagem-dicas-design.md`.
- **Verificado:** baseline limpa antes da documentação: `npm test` com **48 arquivos / 518 testes**, `npx tsc --noEmit` limpo depois do build, `npx eslint .` com **0 erros e 21 warnings** antigos e `npm run build` limpo. O primeiro build falhou apenas porque o sandbox bloqueou Google Fonts; repetido com rede, passou. `next-env.d.ts` foi restaurado depois do build.
- **Em andamento:** revisão do dono sobre a especificação; nenhum código do app foi alterado.
- **Não commitado:** a especificação e este handoff serão commitados juntos no fechamento desta etapa documental.
- **Bloqueado / a decidir:** implementação aguarda a revisão explícita do documento pelo dono, conforme o portão de brainstorming. Pendências anteriores de QA continuam válidas e fora desta etapa.
- **Próximo passo:** após aprovação do documento, criar o plano executável por tarefas L0–L5, com TDD e checks separados para caminho normal e triste.
