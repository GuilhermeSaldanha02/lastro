...
- **O que foi feito:** Tasks L0–L5 de consistência linguística concluídas. Investigado o CI do PR #263: a primeira falha foi uma fixture E2E procurando rótulos antigos depois da localização; corrigidos seis seletores em `e2e/helpers/vinculo.ts`, `e2e/j8-casca-do-personal.spec.ts` e `e2e/j9-caminho-triste.spec.ts`, no commit `2daf299`.
- **Verificado:** `npm run check:i18n` limpo na base; nesta correção, `npm test` com **51 arquivos / 646 testes**, `npm run build`, `npx tsc --noEmit`, lint dos arquivos alterados e `git diff --check` passaram. `next-env.d.ts` foi restaurado após o build. `node scripts/qa-obsoletos.mjs` ainda devolve falsamente “Nao e um repositorio git” neste ambiente.
- **Em andamento:** aguarda envio da branch de correção e CI remoto; E2E local continua proibido porque usa produção.
- **Não commitado:** nada.
- **Bloqueado / a decidir:** push e PR da correção dependem de autorização explícita; a auditoria visual independente e as evidências cruas `print.png`, `console.txt` e `rede.txt` por ID LG continuam pendentes.
- **Próximo passo:** com autorização, enviar `fix/ci-e2e-idiomas`, abrir PR contra `feat/consistencia-linguagem-ajuda`, aguardar CI e só então atualizar QA/matriz conforme a prova disponível.
