...
- **Última sessão:** 2026-09-13 · agente: claude · branch `fix/peso-vazio-e-id-invalido`, **empilhada: #245 (QA) → #246 (A1/A2) → #247 (M2/M3) → #248 (M4/M5) → #249 (M1) → esta (B1/B2)**. Sem merge; merge na ordem da pilha.
- **O que foi feito:** o dono pediu B1 e B2. (B1) `validarNumerosDaSerie` recusa peso em branco; zero digitado continua valendo. (B2) `ehUuid` (`src/lib/dados/id-valido.ts`) nas quatro buscas por id de URL (`buscarTreino`, `buscarExercicio`, `historicoDoExercicio`, `buscarParecer`): id que não é UUID vira "não encontrado" (404) em vez de erro do Postgres (500). Decisão em `DECISIONS.md` "2026-09-13 (6)".
- **Antes, na mesma sessão (CI de cada PR):** #246 run `34793295556` (A1/A2 e OF-08 verdes); #247 run `34795232035` (M2/M3 verdes); #248 run `34795910058` (M4/M5 verdes, 7 vermelhos); #249 (M1) com CI em andamento no fechamento desta entrada.
- **Verificado localmente:** vitest 487/487 (2 casos novos de peso em `limites-serie.test.ts`, 8 em `id-valido.test.ts`), ESLint e `tsc` limpos. Sem navegador local; a prova é o E2E.
- **Esperado no CI desta branch:** ficam VERDES na `j10` "peso vazio não pode gravar 0 kg" e na `j12` "id que não é UUID ou que não existe responde 404". Restam 5 vermelhos por desenho: B3 (PDF em geração), B4 (coach com pergunta invisível), B5 (anilha 0,001), B6 (estouro de largura — `j11` nome de 10 mil caracteres e `j13` nome gigante).
- **Em andamento:** o CI do PR desta branch e o do #249.
- **Não commitado:** nada.
- **Bloqueado / a decidir — tudo do dono:** (1) índice único parcial em `parecer (usuario_id) where status = 'gerando'` — fecha o M5 de vez; é migração, não aplicada nem escrita; (2) merge da pilha, na ordem; (3) auditoria independente das correções — `QA.md` mantém como REPROVOU: TR-06, TR-07, OF-08, TR-08, OF-05, AJ-03, AN-02, OF-04, TR-09, TR-10, CT-02, AN-06; (4) B3 a B8; (5) as 5 contas QA antigas; (6) o que já estava: cápsula TREINO / TRABALHO, PE-06, P1/P2.
- **Próximo passo:** ler o CI deste PR e o do #249.
