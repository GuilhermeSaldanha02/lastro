...
- **Última sessão:** 2026-09-15 · agente: codex · branch: `fix/ci-e2e-idiomas` (a partir de `feat/consistencia-linguagem-ajuda`).
- **O que foi feito:** O CI `35047230414` do PR #264 revelou três expectativas E2E defasadas (404, Home e placeholder do Assistente) e um defeito real no caminho triste: ao sumir a sessão, salvar a meta ficava em “Salvando…”. O commit `184b804` recupera o estado, diferencia sessão conhecida de falha técnica e mantém os dois avisos localizados; também atualiza os contratos E2E estritos conforme a UI renderizada.
- **Verificado:** `npm test` com **51 arquivos / 646 testes**, `npm run build`, `npx tsc --noEmit`, lint dos cinco arquivos e `git diff --check` passaram. `next-env.d.ts` foi restaurado após o build. Revisão independente apontou a classificação errada de toda exceção como sessão expirada; corrigida antes do commit. `node scripts/qa-obsoletos.mjs` rodou e lista a fila histórica completa como obsoleta nesta branch; a prova do item E2E depende do CI.
- **Em andamento:** enviar `184b804` ao PR #264 e aguardar a terceira execução remota. E2E local continua proibido porque usa produção.
- **Não commitado:** atualização deste handoff em `PROGRESS.md`.
- **Bloqueado / a decidir:** aguardar o CI do PR #264; a auditoria visual independente e as evidências cruas `print.png`, `console.txt` e `rede.txt` por ID LG continuam pendentes.
- **Próximo passo:** enviar este commit de handoff, aguardar o CI; se passar, registrar a prova e atualizar QA/matriz; se falhar, investigar a primeira falha real antes de qualquer ajuste.
