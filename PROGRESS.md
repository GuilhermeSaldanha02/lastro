...
2. **Conteúdo do catálogo** (Fase 4) — **fase iniciada em 2026-08-07.** Decisões do dono: (a) catálogo genérico amplo, ~80-100 exercícios, cobrindo todos os grupos musculares — não é a rotina pessoal do dono; (b) separar glúteo, posterior de coxa e panturrilha de "pernas" (hoje todo mundo cai em `quadriceps`). Escopo do que EU posso fazer: nome + grupo muscular + `unilateral` — é dado factual (nome de exercício conhecido), não dica de saúde. **Dica de execução continua vetada pra IA (FF7/ADR-007)** — mesmo pesquisando fontes reais, eu escrevendo o texto conta como "gerado por LLM" e reprova a regra. Toda dica fica `TODO` visível (a tela já trata isso) até o dono (ou fonte curada dele) escrever.
   **Sequenciamento decidido pelo dono:** catálogo amplo primeiro; depois, tela de escolher o(s) grupo(s) musculares do dia (ex.: "peito e ombro", "só perna") ANTES de aparecer a lista de exercícios pra registrar série — vira o próximo item da lista, ainda não iniciado.
...
9. **Seleção de grupo muscular antes de iniciar o treino** (pedido do dono, 2026-08-07) — tela nova, entra DEPOIS do catálogo amplo (item 2) estar pronto, pra o filtro ter o que filtrar de verdade. Ainda não iniciado — sem SDD nem decisão de layout.
