...
- **Última sessão:** 2026-09-14 · agente: claude · **na `main`**, em `b71d523`. A pilha do QA do caminho triste (#245 a #255) foi **mergeada na ordem, com merge commit**, a pedido do dono; depois o #256 (AN-05).
- **O que foi feito (sessão de 2026-09-13 e 14):** QA independente do caminho triste do app inteiro (#245: specs `j10` a `j13`, relatório `qa/relatorio-caminho-triste-2026-09-13.md`) e a correção de todos os achados, um PR por par: #246 A1/A2 (série fora do limite e fila travada em produção), #247 M2/M3 (duplo clique, iniciar treino sem rede), #248 M4/M5 (modelo órfão, análise concorrente), #249 M1 (fila por conta), #250 B1/B2 (peso vazio, id que não é UUID), #251 B3/B4 (PDF em geração, pergunta invisível), #252 B5/B6 (anilha arredondada, texto sem espaço), #253 B7/B8 (fila após login, 404 do app), #254 índice do M5, #255 coach concorrente (achado novo no CI do #251), #256 AN-05 no `QA.md`. Decisões em `DECISIONS.md` "2026-09-13 (2)" a "(11)".
- **Migrações APLICADAS EM PRODUÇÃO nesta sessão, a pedido do dono:** `0027_parecer_uma_geracao_por_usuario` (índice único parcial: uma geração de parecer por conta) e `0028_consumir_uso_ia` (função que conta e grava o uso de IA numa transação só, `security invoker`). Conferidas em `pg_indexes` e `pg_proc`; o verificador do Supabase não acusou nada nelas.
- **Produção:** deploy do merge da pilha (`065c791`) pronto na Vercel; a 404 nova conferida no domínio público.
- **CI:** o topo da pilha (#255, run `34800427546`) passou inteiro — 110 E2E verdes, 0 vermelhos, 47 arquivos de unidade. CI da `main`: run `34802191453` no `b71d523`: em andamento no fechamento desta entrada.
- **Em andamento:** o CI da `main` acima.
- **Não commitado:** nada.
- **Bloqueado / a decidir — tudo do dono:** (1) **auditoria independente das correções**: o `QA.md` mantém como REPROVOU TR-06, TR-07, OF-08, TR-08, OF-05, AJ-03, AN-02, OF-04, TR-09, TR-10, CT-02, AN-06, AN-03, AN-04, AJ-04, VS-06, OF-06 e AN-05 — os specs e as correções são do mesmo agente, então nada disso vira PASSOU sem outra sessão; (2) as 5 contas QA antigas no banco (4 da `j9`, 1 da `j2`); (3) avisos do verificador do Supabase que já existiam: 7 funções `security definer` executáveis por `authenticated` (módulo personal), proteção contra senha vazada desligada, avisos de desempenho de RLS; (4) o que já estava: cápsula TREINO / TRABALHO, PE-06, P1/P2.
- **Próximo passo:** auditoria independente numa sessão separada, começando pelos itens ALTA (TR-06, TR-07, OF-08).
- **Para o outro agente saber:** doze coisas. (11) **Cota de IA se reserva com `consumirUso`** (`src/lib/dados/uso-ia.ts`, função `consumir_uso_ia` da 0028), não com `tetoAtingido` seguido de `registrarUso` — o par solto deixa pedidos simultâneos passarem do teto. A Análise ainda usa o par, coberta pelo índice da 0027. (12) **O resumo do `rtk` para `vitest run` pode mostrar tudo verde com um arquivo de teste inteiro sem carregar**: leia `rtk proxy npx vitest run` e a linha `Test Files`; contagem de testes que cai depois de adicionar teste é arquivo que não carregou. (8) **Em produção, erro lançado numa server action chega ao cliente sem a mensagem**, só com `digest`. Lógica de cliente que decide algo por `error.message` funciona no `next dev` e falha no build (medido no run `34784135645`). A fila offline já foi corrigida (`ResultadoGravacaoSerie`, `DECISIONS.md` 2026-09-13 (2)); informação que o cliente precisa ler volta como valor. (9) **Vermelho em `j10`–`j13` é ACHADO, não teste a ajustar.** Os três defeitos de teste da 1ª rodada estão documentados no relatório; qualquer outro vermelho novo precisa da mesma triagem antes de mexer no spec. (10) **O Chrome do Playwright MCP desta máquina guarda uma credencial salva** (`qa-lastro-perf@example.com`) que não é conta QA conhecida — não usar como instrumento. (1) A `FF5` foi **emendada** — leia a seção abaixo antes de escrever consulta. (2) **A trava da prescrição não está na tela**: está em `src/app/api/analise/route.ts`, ANTES do gasto de cota. (3) **NÃO existem mais contas separadas.** A emenda 2026-09-12 (2) do PRD §11 derrubou o "duas contas" da emenda anterior: toda conta treina, e `tipo_conta = 'personal'` quer dizer "tem área de trabalho". **Os guardas leem `obterPerfil().modo`** (`treino` | `trabalho`), não `tipoConta`. (4) **A própria conta não escreve `tipo_conta` nem `cref`**: GRANT de update por coluna na 0025. Ganhar a área de trabalho é só por `ativar_area_de_trabalho()`. Coluna nova em `usuario` que o app precise escrever exige GRANT novo — sem ele o update falha. (5) **`j4` e `j5` varrem com DUAS contas**; se falharem numa tela sem relação com o módulo, olhe o `beforeAll` antes da tela. (6) **Erro de tela se lê por `.aviso-erro`, nunca `getByRole("alert")`**: o Next injeta `#__next-route-announcer__` com `role="alert"` em toda página. (7) **Sinal de "terminou" em teste tem de ser algo que não pode existir antes** (`DECISIONS.md` "2026-09-12 (1)").
