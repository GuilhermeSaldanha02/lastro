...
**Verificação:** `npx tsc --noEmit` · `npm run test` (133/133) · `npm run lint` (0 erros) · `npm run build` — todos verdes. Chrome real (extensão): `flex-direction: column` computado na linha de modelos; título e meta viram 1 linha cada (`getClientRects().length === 1`), sem quebra no meio da palavra; `Anilhas`/`Coach` inalterados (51,6px de altura, idêntico a antes). Cor computada do botão `Excluir conta`: `rgb(163, 32, 20)` = `#A32014` (o token `--lastro-erro`, conferido por leitura de `tokens.css`). Confirmação em 2 etapas testada dentro do wrapper novo — abre, cancela, conta intacta. Zero erros no console.

**Limite honesto desta verificação.** O ambiente de captura ficou instável de novo nesta sessão — a aba de teste ficou presa em `document.visibilityState: "hidden"` (às vezes com viewport 0×0), precisei fechar e recriar a aba no meio do trabalho pra recuperar um viewport real (~500px, não os 360-390px de iPhone pretendidos). A medição que prova o fix (título/meta caindo pra 1 linha cada) não depende da largura exata — `flex-direction: column` empilha em qualquer largura de tela — mas o dono deveria conferir no aparelho real antes de considerar os dois achados 100% resolvidos.
