...
- **Última sessão:** 2026-09-23 · agente: claude · branch: `docs/estado-pos-auditoria`. Commit atual: rode `git log --oneline -1`.
- **O que foi feito:** (1) **QA de caminho triste de `/treino/[id]` em produção**, conta do dono, 375×812, Playwright MCP, registrado no `QA.md`. TR-06, TR-07, TR-08, TR-09 e TR-10 **corrigidos**; **TR-11 voltou a falhar** (dois toques em "Repetir série" gravam 2 séries: `repetirUltimaSerie` sem trava). Novos: **TR-12** (formulário de série fora da tela e espremido pela casca fixa), **TR-13** (duplo toque em "Finalizar Treino" pula a confirmação: o "Finalizar" da confirmação nasce sob o mesmo ponto), **TR-14** ("12 repetição", `apresentacao-series.ts:36`), **TR-15** (carga por lado sem marca na tabela) e **OF-09** (tela "Sem conexão" não retoma sozinha). A suspeita do 503 no RSC → tela de erro em inglês **não reproduziu** em 16 aberturas; 503 simulado no `/treino?_rsc` cai em carregamento completo, não na tela de erro. (2) [PR #269](https://github.com/GuilhermeSaldanha02/lastro/pull/269) mergeada: **TR-12**. Enquanto o formulário está aberto ou uma série está em edição, a `.acao-area` fica `hidden` (altura 0 publicada); "Fechar" vira ✕ no cabeçalho do cartão (`botao-fechar-cartao.tsx`); abrir o cartão rola até ele. (3) [PR #270](https://github.com/GuilhermeSaldanha02/lastro/pull/270) mergeada e **migração aplicada**: catálogo de **102 para 218** (116 novos, com en/es e dica `claude`), lista em `docs/catalogo-ampliacao-proposta.md`, conferida contra `hasaneyldrm/exercises-dataset` (MIT). Grupo novo **`antebraco`, acessório**: conta volume, mas só entra em `grupos_sem_estimulo` para quem já treinou antebraço (`src/lib/analise/grupos-acessorios.ts`, marcado por quem carrega o catálogo, porque o agregador recebe o grupo já traduzido). Trapézio não virou grupo. (4) [PR #272](https://github.com/GuilhermeSaldanha02/lastro/pull/272) mergeada e migração `20260923160912` aplicada: **12 nomes em inglês viraram português** com o termo conhecido entre parênteses ("Voador (peck deck)", "Desenvolvimento Arnold", "Antirrotação no cabo (Pallof press)"…); o jargão de salão (leg press, stiff, pulley, smith, crossover…) ficou. O player de exercício **sem GIF** agora mostra só o nome sobre o ícone, sem "Animação Ativa", sem "Foco" genérico e sem pedir o `.gif` inexistente. (5) **Mídia:** o dono não vai pagar a licença da Gym Visual; decidiu manter os 102 GIFs com o risco aceito e deixar os 116 novos sem imagem (`DECISIONS.md` 2026-09-23 (3)). (6) [PR #274](https://github.com/GuilhermeSaldanha02/lastro/pull/274) e [PR #275](https://github.com/GuilhermeSaldanha02/lastro/pull/275) mergeadas: **TR-11** (toque duplo em Repetir não grava série a mais) e **TR-13** (toque duplo em Finalizar para na confirmação), pela janela de 500 ms de `src/lib/treino/toque-duplo.ts`; **TR-14** (plural de "repetição"); **TR-15** ("kg/lado" na grade); **OF-09** (tela offline recarrega sozinha; cache do SW em `lastro-offline-v2`). **Auditoria independente** (outro agente, produção, evidência em `qa/evidencias/<ID>/auditoria-independente-2026-09-23/`): **TR-11, TR-12, TR-13, TR-14 e TR-15 PASSOU**; **OF-09 REPROVOU** (a v2 voltava sozinha em só 7 de 14: o `online` chega antes da rede). (7) [PR #277](https://github.com/GuilhermeSaldanha02/lastro/pull/277): OF-09 v3 sonda `/manifest.webmanifest` antes de recarregar e repete a cada 2 s; cache do SW em `lastro-offline-v3`. Medido 14/14 no preview e 4/4 em produção — **ALEGADO**.
- **Verificado:** `vitest` 54 arquivos / 680 testes, `tsc`, `eslint`, `build` nas duas PRs. TR-12 medido no preview com a conta do dono (Registrar em 621–685 com nav em 735; ✕ devolve a área de ações de 151px) — **ALEGADO**, precisa de outro agente. Banco depois da migração: 218 exercícios, 11 grupos, 436 traduções, 0 sem dica, 0 com `unilateral` e `peso_por_lado` juntos. Produção a 375px: catálogo com 218 cards e filtro Antebraço (5), detalhe de exercício novo sem GIF mostra só o nome sobre o ícone, console sem erro (depois da #272); exercício com GIF segue com foco, animação e crédito.
- **Em andamento:** nada.
- **Não commitado:** nada. **Treino de teste `aa0cd9e6-6dc4-477b-9c91-6a9142bd4a59`** (23/09, **6 séries** de Voador 10 × 20 kg: 3 da verificação da #274 e 3 da auditoria independente) ainda está na conta do dono; apagar é com ele.
- **Bloqueado / a decidir:** **mídia Gym Visual sem licença, risco aceito pelo dono** (`DECISIONS.md` 2026-09-23 (3)): não baixar nem publicar mídia nova dela. Saída pronta se houver reclamação: fotos da `yuhonas/free-exercise-db` (Unlicense). Senha de conta QA commitada em `scripts/importar-102-gifs.mjs` (tarefa separada aberta; o dono precisa trocá-la no Supabase). Confirmação da migration `descanso_real_segundos` segue dependendo do E2E manual.
- **Próximo passo:** auditoria independente do OF-09 v3. Depois, o P0 **QA-01**. Observado na auditoria, a decidir com o dono: finalizar treino só grava `localStorage` (outro aparelho vê o treino aberto) e `/treino` segue oferecendo "Continuar treino de hoje" com o treino de hoje já finalizado.
- **Para o outro agente saber:** regras permanentes. (18) **Imagem de exercício nova não vem da Gym Visual** enquanto não houver licença (`DECISIONS.md` 2026-09-23 (3)); exercício sem mídia mostra só o nome. (16) **Grupo muscular novo que quase ninguém treina direto entra em `GRUPOS_ACESSORIOS`**, senão toda Análise o aponta como "sem estímulo": `grupos_sem_estimulo` percorre o catálogo inteiro. (17) **O arquivo de migração tem de ter o nome que o Supabase registrou**: o MCP carimba o próprio timestamp; confira `list_migrations` e renomeie o arquivo, nunca o registro. (15) **Explicação de "o que é tal coisa" entra por `DicaInfo`** (ícone "i" que abre a `Folha`), não como parágrafo fixo. Texto que precisa ser lido antes de agir — consentimento, ação sem volta, confirmação, dado que muda — continua à vista. O botão do "i" nunca vai dentro do `label`. (14) **Senha de conta nova passa por `validarSenhaNova`** (`src/lib/texto/senha.ts`), na tela e no servidor. Fluxo novo de troca ou recuperação de senha precisa usar a mesma função. O aviso de senha vazada do Supabase é aceito: plano gratuito. (13) **Função usada só dentro de policy mora no schema `private`** (0029), fora da API; em `public`, toda função vira endpoint em `/rest/v1/rpc/`. As policies chamam `private.tem_vinculo_aceito`, `private.e_meu_personal` e `private.e_conta_personal`. RPC que o app chama continua em `public`, e o aviso 0029 do Supabase para ela é esperado. (11) **Cota de IA se reserva com `consumirUso`** (`src/lib/dados/uso-ia.ts`, função `consumir_uso_ia` da 0028), não com `tetoAtingido` seguido de `registrarUso` — o par solto deixa pedidos simultâneos passarem do teto. A Análise ainda usa o par, coberta pelo índice da 0027. (12) **O resumo do `rtk` para `vitest run` pode mostrar tudo verde com um arquivo de teste inteiro sem carregar**: leia `rtk proxy npx vitest run` e a linha `Test Files`; contagem de testes que cai depois de adicionar teste é arquivo que não carregou. (8) **Em produção, erro lançado numa server action chega ao cliente sem a mensagem**, só com `digest`. Lógica de cliente que decide algo por `error.message` funciona no `next dev` e falha no build (medido no run `34784135645`). A fila offline já foi corrigida (`ResultadoGravacaoSerie`, `DECISIONS.md` 2026-09-13 (2)); informação que o cliente precisa ler volta como valor. (9) **Vermelho em `j10`–`j13` é ACHADO, não teste a ajustar.** Os três defeitos de teste da 1ª rodada estão documentados no relatório; qualquer outro vermelho novo precisa da mesma triagem antes de mexer no spec. (10) **O Chrome do Playwright MCP desta máquina guarda uma credencial salva** (`qa-lastro-perf@example.com`) que não é conta QA conhecida — não usar como instrumento. (1) A `FF5` foi **emendada** — leia a seção abaixo antes de escrever consulta. (2) **A trava da prescrição não está na tela**: está em `src/app/api/analise/route.ts`, ANTES do gasto de cota. (3) **NÃO existem mais contas separadas.** A emenda 2026-09-12 (2) do PRD §11 derrubou o "duas contas" da emenda anterior: toda conta treina, e `tipo_conta = 'personal'` quer dizer "tem área de trabalho". **Os guardas leem `obterPerfil().modo`** (`treino` | `trabalho`), não `tipoConta`. (4) **A própria conta não escreve `tipo_conta` nem `cref`**: GRANT de update por coluna na 0025. Ganhar a área de trabalho é só por `ativar_area_de_trabalho()`. Coluna nova em `usuario` que o app precise escrever exige GRANT novo — sem ele o update falha. (5) **`j4` e `j5` varrem com DUAS contas**; se falharem numa tela sem relação com o módulo, olhe o `beforeAll` antes da tela. (6) **Erro de tela se lê por `.aviso-erro`, nunca `getByRole("alert")`**: o Next injeta `#__next-route-announcer__` com `role="alert"` em toda página. (7) **Sinal de "terminou" em teste tem de ser algo que não pode existir antes** (`DECISIONS.md` "2026-09-12 (1)").

### Uma conta, dois modos (#239)

Decisão do dono depois do estudo de seis apps da categoria (`DECISIONS.md` "2026-09-12 (2)"). **Toda conta treina.** Com o CREF — pela cápsula PERSONAL no cadastro ou depois, em Ajustes —, a mesma conta ganha a área de trabalho e alterna entre **treino** (Início, Treinos, Análise) e **trabalho** (Fila, Alunos, Catálogo, Ajustes).

- **O seletor é a cápsula TREINO / TRABALHO em Ajustes** (direção A do gate de 2026-09-13), a mesma classe da cápsula do cadastro. Sem área de trabalho, TRABALHO aparece com cadeado e leva a `/personal/completar`; a nota embaixo é o aviso.
- **`/personal/completar` é a porta da área de trabalho** para os dois caminhos: quem já treina e quem nasceu personal sem CREF (Google).
- **Personal pode ter personal.** O próprio convite continua barrado.
- **CREF:** estrito no app e, desde a 0025, **estrito também no banco** (`cref_valido()`), porque só funções `security definer` escrevem a coluna. O app **não verifica** o registro no CONFEF, e toda tela que o mostra diz isso.

### O caminho triste (#238, j9)

Pedido do dono antes de decidir a `main`: só entrada errada. A primeira rodada achou **três defeitos reais**, todos corrigidos no #239: (1) a letra O no lugar do zero sumia do telefone e o app salvava **outro número** — `normalizarTelefoneWhatsApp` agora só descarta pontuação de telefone; (2) o aluno se promovia a personal com um update na própria linha — e a `j8` afirmava isso como correto; (3) CREF `1-G/ZZ` gravado direto pela API. Último CI do módulo: **53 passed**.
...
>
> **Atualização (2026-08-21, sessão seguinte): T2 e T3 fechados; passada da skill Impeccable rodada e registrada.**
>
> - **T2 mergeado** (`5187219`) — `/perfil` e `/ajustes` entraram em `PREFIXOS_PRIVADOS` no `src/proxy.ts`. O 500 em `/ajustes/modelos` sem sessão não acontece mais; testado com `proxy.test.ts` novo.
> - **Impeccable (audit, register product) rodada por pedido do dono, antes de seguir o backlog** — `docs/IMPECCABLE-AUDIT.md`, PR `#103` mergeada. Placar 11/20. Achado principal: `.disciplina-card` e `.metrica-switcher-card` declaravam `backdrop-filter: blur(12px)` com fundo **totalmente opaco** — filtro sem efeito visual, custo de compositor puro. Virou item I1 do backlog de acabamento (`docs/BACKLOG-PROXIMA-FASE.md`), junto com I2–I6 (CSS morto com gradient text, ghost-card em 3 elementos, raio de 22px onde a régua fecha em 16, card aninhado em `/treino`).
> - **T3 fechado, escopo travado pelo dono (opção 2 de duas apresentadas): só o tema padrão.** `--lastro-txt-3` corrigido de `#64748B` para `#7C8DA6` em `tokens.css`. Remedido ao vivo, `/login` e `/catalogo` reais: 4,74 / 5,18 / 5,56 / 5,90 — todos acima do piso 4,5 (antes: 3,36 / 3,95 / 4,19, reprovando nos três). Zero elemento reprovando no DOM onde antes eram 104 (`/catalogo`) e 14 (Home). `DESIGN.md` §3.0–3.2 e §4.2 ganharam banner datado marcando o resto da tabela (16 linhas de §3.2, 13 de C1–C14) como **não remedido contra o Apex Pro** — só a linha `txt-3`/C3 foi corrigida. Entrada em `DECISIONS.md` 2026-08-21. Reconciliação completa das outras 16 linhas e das 6 paletas do card de Tema virou **T3b**, registrado no backlog — não feito agora porque exigiria inventar a razão de decisão de cores que outra sessão escolheu, o que violaria E3.
...
>
> **Atualização (2026-08-16, mesma sessão): H1 fechado (por ora) — `criar modelo` convertido; a premissa antiga de que estava bloqueado por "empilhar hierarquia" não procedia.** Li o código antes de aceitar o bloqueio documentado: os "2 passos" (grupo → exercícios) são estado de cliente puro, sem rota nova — o HIG proíbe outro NÍVEL de navegação dentro da folha, não um componente trocando o que mostra. Risco real (achado pelo `advisor`, não pela premissa antiga): `salvar()` fazia `router.push`, que dentro da folha empilharia uma rota nova por cima da entrada da folha (voltar do navegador cairia de novo nela). Resolvido com uma prop `naFolha` — dentro da folha, salvar fecha com `router.back()` em vez de `push`. Testado ponta a ponta em Chrome real, incluindo o caso do back button pós-save. Resta só `editar série` em H1, decisão documentada de não converter (não pendência). **Só resta H3 (sub-tela) no backlog inteiro.** Bloco detalhado abaixo.
>
> ⚠️ **Correção de processo, registrada por transparência.** Nesta mesma sessão, na PR de `grade de séries` (H2), editei `treino-detalhe.tsx` **antes** de criar a branch — só notei ao rodar `git status` pra commitar. Recuperado sem perda porque `main` não tinha nenhum commit à frente (a branch nasceu com as mudanças já no working tree), mas a regra é branch antes da primeira edição, sempre. Registrado aqui em vez de deixar invisível.
>
...
>
> **Atualização (2026-08-16, mesma sessão): backlog do redesenho fechado — H3 (sub-tela) decidido pelo dono como pendência explícita, sem código.** Sub-tela (lista→detalhe) tem o mesmo limite de D7 que já reduziu a pílula a "só esmaece": sem `ViewTransition` nativo, só dá pra animar a entrada, nunca a saída junto — e um deslize só de entrada teria um efeito colateral visível (tela antiga sumindo de golpe por baixo, em vez de deslizar pra fora). Perguntei ao dono com 3 caminhos (só esmaecer como a pílula / tentar o deslize com o efeito colateral / deixar pendente) — **escolhido deixar pendente**, nenhum código escrito. `/catalogo/[id]` e `/treino/[id]` continuam com troca instantânea. **Com essa decisão, todo item do backlog do redesenho (Trilha B) está FEITO ou FEITO EM PARTE com pendência documentada — nenhum item sem decisão explícita.** Próximo passo, como combinado desde o início da sessão: o teste geral no aparelho físico do dono.
>
> **Atualização (2026-08-16, mesma sessão): pedido avulso do dono, fora da numeração H1-H4 — os 2 bugs visuais achados no iPhone em `/ajustes` (registrados desde o gate do H1) corrigidos.** "Modelos de treino" empilha título/meta em vez de quebrar no meio da palavra (modificador `.item__conteudo--empilhado`, só nessa linha — não a classe base, que outras listas reusam). "Excluir conta" ganhou separação visual (`.zona-risco`, borda acima) e cor destrutiva (`--lastro-erro`, token já existente) em vez de ler como texto solto igual a qualquer link secundário. Bloco detalhado abaixo. **Limite da sessão: ambiente de teste instável de novo** (aba presa em `hidden`/viewport 0×0, precisei recriar a aba no meio da verificação) — a medição que prova o fix (título/meta em 1 linha cada) não depende da largura exata do viewport, mas o dono deveria confirmar no aparelho antes de dar como 100% resolvido.
>
> **Atualização (2026-08-16, mesma sessão): o dono corrigiu minha solução do item 1 — empilhar era feio, não era o que ele queria.** "não ficou bom um em cima do outro, deve acompanhar igual os que já tem na mesma página, se necessário trocar o nome". Revertido `.item__conteudo--empilhado` (removida, zero uso restante) e trocado o texto: "Modelos de treino" → **"Modelos"**, "Montar listas de exercícios" → **"Montar listas"** — volta a ficar lado a lado, igual `Coach`/`Anilhas`. **A extensão Chrome caiu no meio da verificação** (não reconectou nesta sessão); contornado com uma página HTML isolada em `public/` (removida depois), carregando os CSS reais do app, testada a 360px de largura real — as 3 linhas saíram com **74px de altura idênticos**, confirmado também por screenshot. Bloco detalhado abaixo.
>
> **Atualização (2026-08-17, sessão seguinte): pedido avulso do dono, sobre um achado de H1 — a série já preenchida não sinaliza que é editável.** *"não vira folha mais hoje não tem nada mostrando nos card durante o treino que aquilo pode editar depois de preenchido"*. **Isto NÃO reabre a decisão do H1 de manter "editar série" inline** (sem folha) — o dono não pediu isso, só apontou que falta uma pista visual de que a linha reage ao toque. Rejeitei minha primeira ideia (uma frase de instrução única, "toque pra editar") por pedido dele — quis pesquisa de usabilidade de verdade em vez de eu inventar. Pesquisa (`WebSearch`): apps de treino de peso livre (Strong, Hevy) desenham o PRÓPRIO valor de peso×reps como um "chip" — fundo diferenciado, como se fosse um campo — em vez de rótulo ou ícone repetido por linha. Encaixa em D3 (o app já distingue visualmente "navega" de "dado"; isto cria a pista visual que faltava pra um terceiro caso, "dado editável", sem inventar linguagem nova). Implementado: `.serie__v` ganha fundo `--lastro-sup-2` (token já existente, "faixa/cabeçalho de tabela" — reusado, não criado) + `border-radius`; `.serie` ganha `:active` (feedback de toque — antes só reagia a `:hover`, que não existe em celular). Bloco detalhado abaixo.
>
> **Atualização (2026-08-17, mesma sessão): o dono olhou o chip do item acima no aparelho de verdade (durante a auditoria QA geral, em paralelo) e não gostou.** *"não gostei como fica aquela cor mais escura pois o foco é mobile e vi que ficou nada a ver com a proposta, achar outra maneira pra isso."* Fica registrado como **pendência aberta, fora do escopo da auditoria QA em andamento** (aquela é sobre comportamento/robustez, não decisão visual) — não implementado ainda. A verificação que sustentou o merge do PR #80 já tinha o próprio limite documentado ali (CDP travou, sem screenshot real, só medição de DOM isolada) — este é exatamente o tipo de gap que aquele aviso previa: cor medida corretamente (`#E4DACB` = `--lastro-sup-2`) mas a impressão no aparelho real, em contexto (ao lado das outras cores da tela, não isolada), não bateu. Próximo passo: entender com o dono o que especificamente não combina (é a cor em si, o contraste com o resto da tela, ou o conceito de "fundo preenchido" que não existe em nenhum outro lugar do app?) antes de propor alternativa — não redesenhar às cegas.
>
> **Atualização (2026-08-17, mesma sessão): segundo achado do mesmo tipo — o card "Pronto para treinar"/"Como começar?" na Home.** O dono mandou print real do iPhone dele (16:25, com dado real dele — "ontem 25 séries · 8,2t", "2 semanas fechadas com treino") apontando o card escuro como "aquela tela azul". Tentei reproduzir com um usuário de teste isolado antes de saber o que ele via exatamente — não achei nada quebrado, cheguei ao mesmo card. Com o print, confirmado: **não é bug, é o mesmo fundo petróleo escuro** (`--lastro-barra-a/b`, o gradiente da barra de topo) usado no card `.destaque`/"Pronto para treinar" — o dono lê como "azul" e acha que destoa da proposta, mesma categoria da reclamação do chip. Duas peças agora com a mesma queixa (chip de série + card de início), fica registrado junto pra tratar como um pente só quando a pesquisa de usabilidade (pausada, ver `qa-verification-protocol`/brainstorming em andamento) retomar — não é pra tratar como dois problemas isolados.
>
> **Atualização (2026-08-19): o teste geral no aparelho aconteceu — 8 achados, 7 corrigidos e mergeados.** Era a pendência mais antiga em aberto (desde o fim do redesenho). O dono usou o app no iPhone com dado real e mandou print de cada ponto. Backlog em `docs/BACKLOG-TESTE-APARELHO.md` (PR #87), correções em PR próprio por item: **#88** coach (era 503 transitório da Gemini, não bug do app — mas expôs `catch` que não logava nada e não distinguia falha temporária; virou função pura com 8 testes), **#89** linha da lista de modelos sem padding + cápsula branca virando verde, **#90** gráfico espremido (rótulo cortado + janela de 12 semanas com dado só em 2) e esqueleto que lia como caixa vazia, **#91** nome do modelo vira o 1º passo, **#92** anilhas (números boiando + "Salvar" colado em "Adicionar"). Todos com os 4 comandos verdes e verificação ao vivo em **Playwright** — que passou a estar disponível nesta sessão, diferente da auditoria, que teve que usar a extensão Chrome. **Duas premissas corrigidas no caminho, ambas registradas:** o "números soltos" NÃO era a peça M3 estar errada (era a reserva de espaço da lixeira do H2), e o coach NÃO estava quebrado (era o provedor). **Resta o #1** — a cor escura do chip de série, terceira queixa da mesma família junto com o card "Pronto para treinar"; segue guardada por decisão do dono pra uma rodada de pesquisa que trate as três juntas, em vez de uma por vez como no PR #80.
>
> **Atualização (2026-08-17, mesma sessão): auditoria QA geral fechada — Fase 3 rodada (143 itens, 9 áreas), Fase 4 aplicada (4 correções reais mergeadas, 1 achado fechado como não-bug).** Protocolo de 5 fases do dono (ver `qa-verification-protocol`), executado com a extensão Claude in Chrome (Playwright instalado mas indisponível nesta sessão — registrado em `DECISIONS.md` 2026-08-17). PR #81 (test-plan + evidência). Correções, cada uma com PR próprio, `tsc`/`test`/`lint`/`build` verdes e verificação em Chrome real com usuário de teste isolado: **PR #82** contraste ilegível em "Como começar?" (achado avulso do dono, print real, não da auditoria mecânica); **PR #83** foco preso ao abrir "Adicionar exercício" só por teclado (TRANS-07); **PR #84** `/catalogo/[id]` ordenava por `criado_em` em vez da data do treino, quebrando ordem e marcação de recorde (CAT-05/06); **PR #85** validação de formulário nativa bloqueava a mensagem custom do app (REG-06 a REG-11, TDET-04/05). O 5º achado (divergência 3 vs 4 semanas de platô entre `/api/progressao` e `/api/analise`) **investigado e fechado como não-bug** — são dois cálculos propositalmente diferentes (`PLATO_GRAFICO_SEMANAS=3` descritivo/visual vs `SEMANAS_ESTAGNACAO=4` clínico), já documentado em `src/lib/analise/limiares.ts` e `DECISIONS.md` 2026-08-07; dono confirmou não mexer. **Pendências que sobraram, não resolvidas nesta sessão:** a pesquisa de usabilidade do chip de série + card "Pronto para treinar" (pausada, acionar `brainstorming` quando o dono quiser retomar); teste geral no aparelho físico, que já era a pendência anterior à auditoria e continua em aberto.

### ✅ Fix avulso — pista visual de que a série é editável (2026-08-17)

**O pedido do dono, exato.** *"h1 editar serie não vira folha mais hoje não tem nada monstrando nos card durando o treino que aquilo pode editar depois de preenchido e repetir no codigo todo a cada exercicio editar editar, não vai ficar legal."* Duas coisas nessa frase: (1) falta sinal de que a série é editável; (2) repetir a palavra "editar" por linha ficaria feio — rejeitado de antemão.

**Escopo, explícito.** Isto não é reabrir H1 (a decisão de manter "editar série" inline, sem folha, continua valendo — o dono não pediu mudar isso, só apontou a falta de affordance visual).

**Processo — usei a skill `brainstorming` e pesquisa real, não inventei de cara.** Primeira proposta (frase única de instrução, "toque numa série pra editar", mesmo padrão de `.campo__nota`/`.metricas__nota` já usado no app) foi rejeitada pelo dono, que pediu pesquisa de usabilidade em vez de eu simplesmente tentar de novo. `WebSearch` em apps de treino de peso livre reais (Strong, Hevy) — ambos desenham o PRÓPRIO valor (peso×reps) como um "chip": fundo diferenciado do resto da linha, como se fosse um campo de formulário, sem rótulo nem ícone anexado. Mesmo padrão usado pelo Apple Health/Reminders pra "isto é editável" sem texto.

**Por que isso encaixa no lastro especificamente.** D3 já separa visualmente "navega" (moldura + seta) de "dado" (sem moldura, grade) — `.serie` foi construída deliberadamente como dado (M8). O chip cria a pista visual que faltava pra um terceiro caso que o D3 original não previa: "dado que também é editável". Resolve sem contradizer D3 (nenhuma moldura/seta adicionada) e sem repetir texto/ícone por linha (o pedido explícito do dono).

**Implementação, mínima.** `.serie__v` (o valor "12×20kg" inteiro, já um `<span>` só) ganha `background: var(--lastro-sup-2)` (token já existente — "faixa, cabeçalho de tabela" — reusado, nenhum token novo) + `border-radius: var(--lastro-raio-1)` + padding horizontal pequeno. `.serie` ganha `:active` com o mesmo fundo do `:hover` já existente — hoje só reagia a `:hover` (mouse), que nunca aparece em toque; a linha inteira é alvo de toque (não só o chip), então o feedback de toque faltava pra ela toda, não só pro valor.

**Verificação:** `npx tsc --noEmit` · `npm run test` (133/133) · `npm run lint` (0 erros) · `npm run build` — todos verdes. **O dev server caiu no meio da sessão** (porta 3000 sem processo; reiniciado, subiu na 3002) e a extensão Chrome também caiu e voltou — mesma classe de instabilidade de ambiente já registrada antes. Confirmado por medição de DOM no app real (logado, via extensão): cor computada `rgb(228, 218, 203)` = `#E4DACB` = `--lastro-sup-2`, exatamente. Sem screenshot possível ali (CDP travou de novo) — contornado com a mesma técnica já usada pro fix de `/ajustes`: página HTML isolada em `public/` (removida depois) com os CSS reais do app, replicando 2 linhas de `.serie` a 375px de largura real — chip presente nas duas, sem overflow horizontal (`scrollWidth === clientWidth`, 335px, a mesma largura de conteúdo já medida em M5/M8). Zero erros no console.

**Não visto ao vivo.** O efeito não foi visto rodando no navegador de verdade (só medido/replicado) — nem o `:active` de toque, que por natureza só existe durante o gesto e não dá pra capturar por medição estática. O dono deveria conferir no aparelho: o chip aparecendo nos valores, e a linha reagindo ao toque.

### ✅ Fix avulso — 2 bugs visuais em `/ajustes` achados no iPhone (2026-08-16)

**Contexto.** Dois problemas registrados desde o gate visual do H1 (2026-08-15), fora do escopo declarado de qualquer item H1-H4 — pedido do dono depois que o backlog inteiro já estava fechado.

**1. "Modelos de treino" quebrava em 2 linhas e desalinhava.** `.item__conteudo` usa `justify-content: space-between` entre `.atalho__titulo`/`.atalho__meta` (M4) — funciona bem com pares curtos ("Coach"/"Tirar uma dúvida", "Anilhas"/"Configurar e calcular"), mas "Modelos de treino"/"Montar listas de exercícios" é o par mais longo da tela, e sem `min-width:0` nos spans o flexbox reduz até o `min-content` (a palavra mais longa), forçando quebra no meio de "exercícios".

**2. "Excluir conta" lia como solto.** `.pilha > * + *` dá margem uniforme entre TODOS os filhos diretos (perfil, lista, "Sair", "Excluir conta") — trata uma ação irreversível com o mesmo peso visual de qualquer linha comum, sem nenhum sinal de que é diferente. Fix: `src/components/excluir-conta.tsx` ganhou um wrapper `.zona-risco` (`border-top` + `padding-top`, separa de "Sair" com um traço) e o botão de disparo trocou `.botao-textual` por `.botao-textual botao-textual--destrutivo` (cor `--lastro-erro` — o mesmo token já reservado a "falha real e ação destrutiva", reusado, nenhum token novo).

**Correção do item 1, mesmo dia — a 1ª tentativa (empilhar) foi revertida a pedido do dono.** Primeira solução: modificador `.item__conteudo--empilhado` (`flex-direction: column`), aplicado só nessa linha. Funcionava sem quebrar palavra, mas o dono não gostou do resultado visual — "não ficou bom um em cima do outro, deve acompanhar igual os que já tem na mesma página, se necessário trocar o nome". Diagnóstico da causa (par longo demais) estava certo; a solução (mudar o layout) que não era a certa — a consistência da lista importava mais que preservar o texto original. Corrigido de verdade encurtando: "Modelos de treino" → **"Modelos"**, "Montar listas de exercícios" → **"Montar listas"**, voltando ao layout lado a lado padrão. `.item__conteudo--empilhado` removida de `sistema.css` (zero uso restante no app).

**Verificação (texto encurtado, versão final):** `npx tsc --noEmit` · `npm run test` (133/133) · `npm run lint` (0 erros) · `npm run build` — todos verdes. **A extensão Chrome caiu no meio desta sessão e não reconectou** — sem tab autenticado disponível, contornei criando uma página HTML isolada (`public/_teste_linha.html`, removida ao final, junto com cópias temporárias de `tokens.css`/`sistema.css` também em `public/`) com as 3 linhas lado a lado (Coach, Modelos, Anilhas) usando os CSS reais do app. Faltou `<meta name="viewport">` na primeira tentativa — sem ela o navegador assume 980px de largura por padrão e mascararia qualquer teste de quebra; corrigido antes de medir. A 360px reais: as três linhas saíram com **74px de altura idênticos**, título e meta em 1 linha cada nas três — confirmado por medição de DOM E por screenshot real (não só números). Cor do botão "Excluir conta" (item 2, já resolvido antes desta correção) segue válida: `rgb(163, 32, 20)` = `#A32014`, o token `--lastro-erro`.

**Limite honesto:** a verificação desta correção final rodou numa página de teste isolada, não na rota `/ajustes` de verdade dentro do app (que exige login, indisponível sem a extensão) — o CSS testado é byte-a-byte o mesmo que o app usa, mas o dono ainda deveria abrir `/ajustes` no aparelho pra confirmar no contexto real.
...
### ✅ H1 — `criar modelo` vira folha; correção de uma premissa nunca verificada (2026-08-16)

**A premissa que caiu.** O registro anterior dizia: `criar modelo` tem 2 passos (escolher grupo → escolher exercícios) e "empilhar passos dentro de uma folha é o que o próprio item proíbe, 'não empilhar hierarquia'". Antes de aceitar isso como bloqueio de verdade, li `modelo-treino-form.tsx`: os dois passos são só `gruposEscolhidos.length === 0` decidindo o que renderizar, e "Trocar grupo" é só `setGruposEscolhidos([])` — nenhuma rota nova, nenhuma entrada de histórico. O HIG proíbe apresentar OUTRO NÍVEL de navegação dentro da folha (uma segunda folha, um drill-down com voltar próprio) — não um componente trocando o que mostra por estado. Mesma disciplina da correção de D7 nesta sessão: verificar por leitura de código antes de aceitar uma nota antiga como verdade.

**Consultei o `advisor` mesmo assim antes de codar** (Nível 3, reverte um bloqueio já documentado) — confirmou a leitura e apontou o risco real, que não tinha nada a ver com a premissa antiga: `salvar()` fazia `router.push("/ajustes/modelos")` no sucesso. Dentro de uma folha (rota interceptada empilhada sobre `/ajustes/modelos`), um `push` empurraria mais uma rota por cima da entrada da folha — o voltar do navegador cairia de novo nela, num modelo que já foi criado.

**Resolvido com uma prop nova, `naFolha`** (default `false`, sem mudar o comportamento da rota cheia): dentro da folha, salvar fecha com `router.back()` — o mesmo mecanismo de histórico que já fecha por ✕/Esc/arraste — em vez de empurrar rota nova. `criarModelo` já chama `revalidatePath("/ajustes/modelos")`, então a lista se atualiza sozinha ao voltar. Rota nova: `src/app/@modal/(.)ajustes/modelos/novo/page.tsx`, mesmo padrão de `(.)ajustes/anilhas` (marcador `(.)` conta a partir da raiz de `app/`, ignora o slot `@modal`).

**Verificação:** `npm run build` (primeiro, regenera tipos de rota interceptada) → `npx tsc --noEmit` → `npm run test` (133/133) → `npm run lint` (0 erros) — todos verdes. Chrome real (extensão, sessão logada), fluxo ponta a ponta: "criar modelo" abre a folha a partir de `/ajustes/modelos`; passo 1 → "Continuar" → passo 2, sem sair de `/ajustes/modelos/novo`; salvar fecha a folha, volta pra `/ajustes/modelos`, modelo aparece na lista; **botão voltar do navegador depois do save não reabre a folha** (o risco específico que o `advisor` apontou, testado e confirmado ausente); acesso direto por URL continua caindo limpo na rota cheia, fallback intocado. Modelo de teste criado e removido ao final. Zero erros no console.

**Não visto ao vivo:** rolagem da folha com teclado do celular aberto sobre lista longa de exercícios — só a estrutura (`overflow-y:auto`) foi exercitada via DOM.

**H1 considerado fechado por ora — 3 dos 4 fluxos convertidos** (`editar perfil`, `adicionar anilha`, `criar modelo`); o 4º (`editar série`) é decisão documentada de não converter, não pendência esquecida.
