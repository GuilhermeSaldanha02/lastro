...
- **O que foi feito:** o caminho triste do app inteiro, pedido do dono. Quatro specs novos com 52 casos: `e2e/j10-serie-e-fila`, `j11-formularios`, `j12-isolamento-e-apis`, `j13-sessao-e-navegacao`, mais o helper `e2e/helpers/caminho-triste.ts`. Checagem sem conta direto na produção (`qa/evidencias/TR-PUB-01/`). **Relatório completo: `qa/relatorio-caminho-triste-2026-09-13.md`.** `QA.md` ganhou 23 itens (TR-06 a VS-08).
- **Resultado da 1ª rodada (run `34784135645`):** 89 passed, 19 failed, 1 flaky. Triagem: **16 achados reais e 3 defeitos do próprio teste**, corrigidos no commit `10f51ba`. **2ª rodada: run `34785642721`, em andamento no fechamento desta entrada.**
- **Os dois achados ALTA, para ler antes de mexer na fila offline:** (A1) série com reps 201, reps 2,5, peso > 1000 ou RIR 1,5 passa pela tela, fica PENDENTE para sempre e some ao recarregar — o build de produção troca a mensagem da server action por `digest`, o prefixo `[erro-permanente]` se perde e o item nunca vai para `db.falhas` (o OF-02 não vale em produção); (A2) editar para reps 999 mostra 999 e o banco guarda o valor antigo.
...
- **Em andamento:** a 2ª rodada do CI no PR #245.
- **Não commitado no fechamento desta entrada:** `QA.md`, `PROGRESS.md`, o relatório e as evidências (`qa/evidencias/TR-06` … `VS-06`, `log-ci-run-34784135645.txt`) — entram no próximo commit da branch.
- **Bloqueado / a decidir — tudo do dono:** (1) **triar os 16 achados** e decidir o que corrigir e em que ordem; nenhum foi corrigido, de propósito; (2) merge do PR #245 — com os achados abertos, o CI dele fica VERMELHO por desenho até as correções; (3) as 5 contas QA antigas no banco (as 4 da `j9` de hoje e uma da `j2` de 2026-09-05, esta última nova para a lista); (4) o que já estava: olho na cápsula TREINO / TRABALHO e nas cascas, PE-06, perguntas aos personais P1/P2.
