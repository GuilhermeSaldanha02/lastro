...
- **Última sessão:** 2026-09-16 · agente: codex · branch: `codex/redesenho-tabela-treino` (rebaseada sobre `main`).
- **O que foi feito:** Redesign estrutural de `/treino/[id]` concluído: grade contínua de séries, tipos AQ/VAL e recorde localizados, descanso realmente medido e exibido na série anterior, timer do topo ligado ao mesmo estado e persistência FIFO offline. O indicador agora volta a “salvo no aparelho” enquanto a atualização do descanso aguarda sincronização.
- **Verificado:** pós-rebase: `npm test` com **54 arquivos / 675 testes**, `npx tsc --noEmit`, `npx eslint .`, `npm run build` e `git diff --check` passaram; `next-env.d.ts` foi restaurado após o build. E2E local não foi executado porque usa produção.
- **Em andamento:** publicar a PR do redesign e aguardar CI/Vercel; depois, conferir visualmente no preview mobile a grade, o timer e o caminho de descanso.
- **Não commitado:** atualização deste handoff em `PROGRESS.md`.
- **Bloqueado / a decidir:** a CLI/credenciais do Supabase não estão nesta máquina, então a confirmação da migration `descanso_real_segundos` virá no CI contra o banco hospedado.
- **Próximo passo:** push, PR, CI e gate visual no preview; se algum check falhar, investigar a primeira causa antes de alterar código.
