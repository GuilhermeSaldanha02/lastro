...
- **Última sessão:** 2026-09-13 · agente: claude · branch `docs/personal-estado-modos`, tirada de `feat/modulo-personal` em `9b76ddc`. Nenhum PR aberto além deste, working tree limpa.
- **A `feat/modulo-personal` tem DOZE merges:** #226 vínculo e fila · #228 prescrição escondida · #229 cobertura da fila · #230 medida da §11.7 · #231 varreduras · #232 fechamento de registro · #233 conta de personal com CREF e a cápsula no cadastro · #234 a casca do personal · #235 registro da casca · #236 tradução das strings da prescrição · **#238 caminho triste (j9)** · **#239 uma conta, dois modos**. A `main` segue **intocada** em `4a4839f` — só recebe o módulo inteiro (`AGENTS.md` §4).
- **Migração 0025 APLICADA em produção** em 2026-09-13, com confirmação explícita do dono. A `main` não lê `modo_ativo` nem escreve `tipo_conta`/`cref`, e as colunas que ela escreve em `usuario` continuam com GRANT.
- **Em andamento:** nada.
- **Não commitado:** nada.
- **Bloqueado / a decidir — tudo do dono:** (1) quando o módulo vai para a `main`; (2) **o olho na cápsula TREINO / TRABALHO no celular**, inclusive no tema claro Marfim & Ouro — QA dela é **ALEGADO** (nenhum agente a viu renderizada; a borda passou a `--lastro-controle` depois da auditoria contra a `main`, e nenhum teste mede contraste de borda); (3) se PASSOU por determinismo de CI encerra o `AGENTS.md` §5; (4) PE-06, a resposta real da Gemini sob vínculo.
- **Próximo passo:** o dono abrir o preview `https://lastro-git-feat-modulo-personal-audicon.vercel.app` (Supabase de PRODUÇÃO), olhar Ajustes com a própria conta (TRABALHO travado + aviso) e decidir a `main`. Não há trabalho AFK pendente.
- **Para o outro agente saber:** sete coisas. (1) A `FF5` foi **emendada** — leia a seção abaixo antes de escrever consulta. (2) **A trava da prescrição não está na tela**: está em `src/app/api/analise/route.ts`, ANTES do gasto de cota. (3) **NÃO existem mais contas separadas.** A emenda 2026-09-12 (2) do PRD §11 derrubou o "duas contas" da emenda anterior: toda conta treina, e `tipo_conta = 'personal'` quer dizer "tem área de trabalho". **Os guardas leem `obterPerfil().modo`** (`treino` | `trabalho`), não `tipoConta`. (4) **A própria conta não escreve `tipo_conta` nem `cref`**: GRANT de update por coluna na 0025. Ganhar a área de trabalho é só por `ativar_area_de_trabalho()`. Coluna nova em `usuario` que o app precise escrever exige GRANT novo — sem ele o update falha. (5) **`j4` e `j5` varrem com DUAS contas**; se falharem numa tela sem relação com o módulo, olhe o `beforeAll` antes da tela. (6) **Erro de tela se lê por `.aviso-erro`, nunca `getByRole("alert")`**: o Next injeta `#__next-route-announcer__` com `role="alert"` em toda página. (7) **Sinal de "terminou" em teste tem de ser algo que não pode existir antes** (`DECISIONS.md` "2026-09-12 (1)").
