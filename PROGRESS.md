...
- **Última sessão:** 2026-09-14 · agente: claude · branch `feat/icone-info-avisos` (#260), atualizada com a `main` em `c670801` depois do merge do #259 (senha forte no cadastro). Merge do #260 pedido pelo dono assim que o CI desta atualização passar e o print da folha aberta sair certo.
- **O que foi feito:** o dono pediu trocar os textos explicativos por um ícone com popup e aprovou a divisão. `DicaInfo` (`src/components/dica-info.tsx`): ícone "i" ao lado do título ou do rótulo, abre a `Folha` com o texto. A `Folha` ganhou `onFechar` e `focarAoAbrir`. Explicações de 12 arquivos viraram ícone; confirmação de salvar, dado que muda, aviso de formato e consentimento ficaram à vista. Decisão em `DECISIONS.md` "2026-09-14 (3)".
- **Já na `main`, na mesma sessão:** #258 (helpers das policies no schema `private`, migração 0029 aplicada em produção) e #259 (senha forte no cadastro, `validarSenhaNova`, decisão "2026-09-14 (2)").
- **Ajuste desta atualização:** a primeira rodada do CI do #260 (run `34805657227`, 111 verdes) fotografou a folha no meio da animação de entrada, semitransparente por cima do formulário. A `j14` agora espera as animações do fundo e da folha terminarem e confirma opacidade 1 antes do print `dica-aberta`. Conflito do merge com a `main` resolvido: as duas importações em `src/app/login/page.tsx`, as duas decisões em ordem e as duas notas novas desta lista.
- **Verificado localmente:** vitest, ESLint e `tsc` depois da resolução do conflito. **Sem navegador local**: a prova visual são os prints do CI.
- **Em andamento:** o CI desta atualização do #260.
- **Não commitado:** nada.
- **Bloqueado / a decidir — tudo do dono:** (1) auditoria independente das correções do QA — o `QA.md` mantém 18 itens como REPROVOU; (2) o que já estava: cápsula TREINO / TRABALHO, PE-06, P1/P2.
- **Próximo passo:** CI verde e print da folha aberta certo, então merge do #260.
