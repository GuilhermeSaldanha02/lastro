...
- **Última sessão:** 2026-09-13 · agente: claude · branch `fix/duplo-clique-e-iniciar-sem-rede`, **empilhada sobre `fix/serie-fora-do-limite` (PR #246), que está sobre `chore/qa-caminho-triste-app` (PR #245)**. Sem merge; ordem de merge: #245, #246, este.
- **O que foi feito:** o dono pediu M2 e M3. (M2) `formulario-serie.tsx` ganhou trava de envio (ref + botão desabilitado). (M3) novo `src/components/form-iniciar-treino.tsx`, usado na Home, em `/treino` e em `iniciar-treino.tsx`: sem rede mostra aviso em vez de chamar o servidor, falha de rede vira aviso, `redirect()` segue por `unstable_rethrow`. Decisão em `DECISIONS.md` "2026-09-13 (3)".
- **Antes, na mesma sessão:** A1/A2 corrigidos no #246 (`DECISIONS.md` "2026-09-13 (2)"); CI dele (run `34793295556`) com os 7 casos de A1/A2/OF-08 verdes e só os 13 achados restantes vermelhos.
- **Verificado localmente:** ESLint e `tsc` limpos. Sem teste de unidade novo (a mudança é de componente) e sem navegador local (sem `.env.local`); a prova é o E2E no CI.
- **Esperado no CI desta branch:** ficam VERDES na `j10` "duplo clique em 'Registrar série'" e "'Iniciar treino' sem rede"; `j1` (inicia treino com rede) tem de seguir verde. Restam 11 achados vermelhos por desenho.
- **Em andamento:** o CI do PR desta branch.
- **Não commitado:** nada.
- **Bloqueado / a decidir — tudo do dono:** (1) merge dos três PRs, na ordem; (2) próxima leva: sugestão M4 (modelo órfão) e M5 (análise com cota dupla); (3) auditoria independente das correções — `QA.md` mantém TR-06, TR-07, OF-08, TR-08 e OF-05 como REPROVOU; (4) as 5 contas QA antigas; (5) o que já estava: cápsula TREINO / TRABALHO, PE-06, P1/P2.
- **Próximo passo:** ler o CI deste PR caso a caso (quais viraram, quais seguem).
