...
- **Última sessão:** 2026-09-15 · agente: codex · branch: `feat/consistencia-linguagem-ajuda`.
- **O que foi feito:** Tasks L0–L5 de consistência linguística concluídas: portão de literais, matriz de caminhos normal/triste, superfícies de aluno, personal, catálogo, casca global, análise/assistente/relatórios e automação de cenários.
- **Verificado:** localmente em 2026-09-15: `npm run check:i18n` limpo; `npx tsc --noEmit` limpo; `npm test` com **51 arquivos / 646 testes**; `npx eslint .` limpo; `npm run build` concluído. `next-env.d.ts` foi restaurado após o build. `node scripts/qa-obsoletos.mjs` ainda devolve falsamente “Nao e um repositorio git” neste ambiente.
- **Em andamento:** fechamento honesto da QA local; nenhuma célula foi promovida a `ALEGADO` ou `PASSOU` sem os três artefatos obrigatórios.
- **Não commitado:** atualização deste handoff de QA.
- **Bloqueado / a decidir:** push, PR e CI ainda não executados; E2E local é proibido porque usa produção. Auditoria visual independente nas seis células (375 px e ampla) continua pendente, assim como as evidências cruas `print.png`, `console.txt` e `rede.txt` por ID LG.
- **Próximo passo:** após autorização de push, abrir PR, conferir o CI e executar a auditoria independente; só então atualizar QA/matriz conforme a prova disponível.
