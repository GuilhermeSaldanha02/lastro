...
- **Última sessão:** 2026-09-13 · agente: claude · branch `fix/modelo-orfao-e-analise-concorrente`, **empilhada: #245 (QA) → #246 (A1/A2) → #247 (M2/M3) → esta (M4/M5)**. Sem merge; merge na ordem da pilha.
- **O que foi feito:** o dono pediu M4 e M5. (M4) `lerPlanoDoModelo` em `src/lib/dados/limites-modelo.ts` recusa o plano fora do limite de `modelo_treino_exercicio` no formulário, e `criarModelo` apaga o cabeçalho quando os itens falham. (M5) `POST /api/analise` grava o rascunho, desempata pelo mais antigo em `gerando` e só o vencedor registra uso; o perdedor apaga o rascunho e responde 409. Decisão em `DECISIONS.md` "2026-09-13 (4)".
- **Antes, na mesma sessão:** #246 (A1/A2, CI `34793295556` com os 7 casos verdes) e #247 (M2/M3).
- **Verificado localmente:** vitest 472/472 (9 casos novos em `limites-modelo.test.ts`), ESLint e `tsc` limpos. Sem navegador local (sem `.env.local`); a prova é o E2E no CI.
- **Esperado no CI desta branch:** ficam VERDES na `j11` os 3 casos "plano com ... não pode deixar um modelo vazio" e na `j12` "dois pedidos simultâneos não criam dois rascunhos"; tem de seguir verde na `j12` "em andamento responde 409 e teto responde 429, sem gastar cota". Restam 7 achados vermelhos por desenho (B2 a B7 e o nome gigante).
- **Em andamento:** o CI do PR desta branch.
- **Não commitado:** nada.
- **Bloqueado / a decidir — tudo do dono:** (1) **índice único parcial em `parecer (usuario_id) where status = 'gerando'`** — é o que torna o M5 à prova de corrida; é migração, não foi aplicada nem escrita; (2) merge da pilha, na ordem; (3) auditoria independente das correções — `QA.md` mantém TR-06, TR-07, OF-08, TR-08, OF-05, AJ-03 e AN-02 como REPROVOU; (4) as 5 contas QA antigas; (5) o que já estava: cápsula TREINO / TRABALHO, PE-06, P1/P2.
- **Próximo passo:** ler o CI deste PR caso a caso.
