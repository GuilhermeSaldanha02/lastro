...
- **Última sessão:** 2026-09-13 · agente: claude (QA INDEPENDENTE, não escreveu o código testado) · branch `chore/qa-caminho-triste-app`, tirada da `main` em `475c774` · **PR #245 aberto, sem merge** (merge só quando o dono pedir).
- **O que foi feito:** o caminho triste do app inteiro, pedido do dono. Quatro specs novos com 52 casos: `e2e/j10-serie-e-fila`, `j11-formularios`, `j12-isolamento-e-apis`, `j13-sessao-e-navegacao`, mais o helper `e2e/helpers/caminho-triste.ts`. Checagem sem conta direto na produção (`qa/evidencias/TR-PUB-01/`). **Relatório completo: `qa/relatorio-caminho-triste-2026-09-13.md`.** `QA.md` ganhou 26 itens (TR-06 a VS-08, CT-02, AN-06, OF-08); todo PASSOU com `qa/evidencias/<ID>/` e a ressalva de determinismo de CI.
- **Resultado da 1ª rodada (run `34784135645`):** 89 passed, 19 failed, 1 flaky. Triagem: **15 achados reais e 3 defeitos do próprio teste**, corrigidos no commit `10f51ba`. **2ª rodada (run `34785642721`, commit `10f51ba`): 90 passed, 20 failed, 0 flaky** — os 20 são achados: M3 agora medido certo, o login com e-mail gigante passou (não é achado), o M1 passou de primeira (intermitente, 1 falha em 3) e o caso novo provou a consequência do A1 (OF-08).
- **Os dois achados ALTA, para ler antes de mexer na fila offline:** (A1) série com reps 201, reps 2,5, peso > 1000 ou RIR 1,5 passa pela tela, fica PENDENTE para sempre e some ao recarregar, e **trava a fila: a série válida registrada depois também nunca sincroniza** (OF-08, medido nas duas tentativas) — o build de produção troca a mensagem da server action por `digest`, o prefixo `[erro-permanente]` se perde e o item nunca vai para `db.falhas` (o OF-02 não vale em produção); (A2) editar para reps 999 mostra 999 e o banco guarda o valor antigo.
- **MÉDIA:** fila de aparelho compartilhado travada por série de outra conta (intermitente); duplo clique em "Registrar série" grava 2; "Iniciar treino" sem rede troca a tela pela página de erro do Next; modelo com plano fora do limite deixa modelo vazio no banco; dois POST simultâneos à Análise gastam cota dupla.
- **PASSOU, com prova:** isolamento completo entre duas contas sem relação (API com o próprio JWT, rotas por id, PDF, CSV, progressão, storage), 401 sem sessão, `?proximo=` externo, botão voltar depois de sair e de excluir, 409/429 sem cota, ciclo offline criar/editar/excluir.
- **Em andamento:** a 3ª execução do CI (run `34787527564`, commit `63911c9`, só documentação) disparada pelo push final — é o status visível no PR #245 e deve repetir as 20 falhas da 2ª rodada. A evidência dos achados é a run `34785642721`. O CI do PR fica vermelho por desenho enquanto os achados estiverem abertos.
- **Não commitado:** nada; relatório, `QA.md`, evidências e este bloco estão commitados e com push na branch.
- **Bloqueado / a decidir — tudo do dono:** (1) **triar os 15 achados** (A1, A2, M1–M5, B1–B8; o `QA.md` tem 18 linhas REPROVOU porque o B2 virou três áreas e a consequência do A1 ganhou a linha OF-08) e decidir o que corrigir e em que ordem; nenhum foi corrigido, de propósito; (2) merge do PR #245 — com os achados abertos, o CI dele fica VERMELHO por desenho até as correções; (3) as 5 contas QA antigas no banco (as 4 da `j9` de hoje e uma da `j2` de 2026-09-05, esta última nova para a lista); (4) o que já estava: olho na cápsula TREINO / TRABALHO e nas cascas, PE-06, perguntas aos personais P1/P2.
- **Próximo passo:** o dono lê o relatório e prioriza. Sugestão do QA: A1 e A2 primeiro (validar os limites de `serie` na tela e não depender da mensagem do erro atravessar a server action), depois o duplo clique e a tela sem rede.
- **Para o outro agente saber:** dez coisas. (8) **Em produção, erro lançado numa server action chega ao cliente sem a mensagem**, só com `digest`. Toda lógica de cliente que decide algo por `error.message` (hoje: `ehErroPermanente` da fila offline) funciona no `next dev` e falha no build. Medido no run `34784135645`. (9) **Vermelho em `j10`–`j13` é ACHADO, não teste a ajustar.** Os três defeitos de teste da 1ª rodada estão documentados no relatório; qualquer outro vermelho novo precisa da mesma triagem antes de mexer no spec. (10) **O Chrome do Playwright MCP desta máquina guarda uma credencial salva** (`qa-lastro-perf@example.com`) que não é conta QA conhecida — não usar como instrumento. (1) A `FF5` foi **emendada** — leia a seção abaixo antes de escrever consulta. (2) **A trava da prescrição não está na tela**: está em `src/app/api/analise/route.ts`, ANTES do gasto de cota. (3) **NÃO existem mais contas separadas.** A emenda 2026-09-12 (2) do PRD §11 derrubou o "duas contas" da emenda anterior: toda conta treina, e `tipo_conta = 'personal'` quer dizer "tem área de trabalho". **Os guardas leem `obterPerfil().modo`** (`treino` | `trabalho`), não `tipoConta`. (4) **A própria conta não escreve `tipo_conta` nem `cref`**: GRANT de update por coluna na 0025. Ganhar a área de trabalho é só por `ativar_area_de_trabalho()`. Coluna nova em `usuario` que o app precise escrever exige GRANT novo — sem ele o update falha. (5) **`j4` e `j5` varrem com DUAS contas**; se falharem numa tela sem relação com o módulo, olhe o `beforeAll` antes da tela. (6) **Erro de tela se lê por `.aviso-erro`, nunca `getByRole("alert")`**: o Next injeta `#__next-route-announcer__` com `role="alert"` em toda página. (7) **Sinal de "terminou" em teste tem de ser algo que não pode existir antes** (`DECISIONS.md` "2026-09-12 (1)").
