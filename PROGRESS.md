...
- **Última sessão:** 2026-09-13 · agente: claude · branch `fix/pdf-em-geracao-e-pergunta-invisivel`, **empilhada: #245 (QA) → #246 (A1/A2) → #247 (M2/M3) → #248 (M4/M5) → #249 (M1) → #250 (B1/B2) → esta (B3/B4)**. Sem merge; merge na ordem da pilha.
- **O que foi feito:** o dono pediu B3 e B4. (B3) `/api/parecer/[id]/pdf` responde 404 para rascunho sem texto ou evidência. (B4) `limparPergunta` em `src/app/api/coach/prompt.ts` tira caracteres invisíveis (Unicode Cf, U+2028/U+2029) antes de medir a pergunta; vazia vira 400 sem gastar cota; a tela do coach usa a mesma função. Decisão em `DECISIONS.md` "2026-09-13 (7)".
- **Antes, na mesma sessão (CI de cada PR):** #246 run `34793295556` (A1/A2 e OF-08 verdes); #247 run `34795232035` (M2/M3 verdes); #248 run `34795910058` (M4/M5 verdes); #249 (M1) e #250 (B1/B2) com CI em andamento no fechamento desta entrada.
- **Verificado localmente:** vitest 47 arquivos e 496 testes (contagem crua), ESLint e `tsc` limpos. Sem navegador local; a prova é o E2E.
- **Esperado no CI desta branch:** ficam VERDES na `j12` "PDF: parecer ainda em geração" e "coach: pergunta invisível". Restam 3 vermelhos por desenho: B5 (`j11` anilha 0,001) e B6 (`j11` nome de 10 mil caracteres, `j13` nome gigante).
- **Em andamento:** o CI deste PR, do #249 e do #250.
- **Não commitado:** nada.
- **Bloqueado / a decidir — tudo do dono:** (1) índice único parcial em `parecer (usuario_id) where status = 'gerando'` — fecha o M5 de vez; é migração, não aplicada nem escrita; (2) merge da pilha, na ordem; (3) auditoria independente das correções — `QA.md` mantém como REPROVOU: TR-06, TR-07, OF-08, TR-08, OF-05, AJ-03, AN-02, OF-04, TR-09, TR-10, CT-02, AN-06, AN-03, AN-04; (4) B5 a B8; (5) as 5 contas QA antigas; (6) o que já estava: cápsula TREINO / TRABALHO, PE-06, P1/P2.
- **Próximo passo:** ler o CI deste PR, do #249 e do #250.
