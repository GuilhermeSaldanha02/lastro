...
- **Última sessão:** 2026-09-13 · agente: claude · branch `fix/anilha-minima-e-texto-sem-espaco`, **empilhada: #245 (QA) → #246 (A1/A2) → #247 (M2/M3) → #248 (M4/M5) → #249 (M1) → #250 (B1/B2) → #251 (B3/B4) → esta (B5/B6)**. Sem merge; merge na ordem da pilha.
- **O que foi feito:** o dono pediu B5 e B6. (B5) `normalizarPesoKg` em `src/lib/anilhas.ts` arredonda o peso de barra e de anilha para duas casas antes de validar (0,01 a 9999,99 kg), na tela e em `salvarConfigAnilhas`. (B6) `overflow-wrap: anywhere` em `.item__estatico` (lista de modelos) e na nova `.perfil-nome` (`/perfil`). Decisão em `DECISIONS.md` "2026-09-13 (8)".
- **Antes, na mesma sessão (CI de cada PR):** #246 run `34793295556` (A1/A2 e OF-08 verdes); #247 run `34795232035` (M2/M3 verdes); #248 run `34795910058` (M4/M5 verdes); #249 run `34797258298` (M1), #250 run `34797495882` (B1/B2) e #251 run `34797779114` (B3/B4) em andamento no fechamento desta entrada.
- **Verificado localmente:** vitest 47 arquivos e 505 testes (contagem crua; 9 casos novos de `normalizarPesoKg`), ESLint e `tsc` limpos. Sem navegador local; a prova visual do B6 são os prints do E2E nos artefatos do CI.
- **Esperado no CI desta branch:** verdes "anilha de 0,001 kg" e "nome de 10 mil caracteres" (`j11`) e "nome com HTML e milhares de caracteres" (`j13`). Com isso, **nenhum vermelho por desenho deve sobrar**: todo achado com teste automatizado estaria corrigido.
- **Em andamento:** o CI deste PR e dos #249, #250 e #251.
- **Não commitado:** nada.
- **Bloqueado / a decidir — tudo do dono:** (1) índice único parcial em `parecer (usuario_id) where status = 'gerando'` — fecha o M5 de vez; é migração, não aplicada nem escrita; (2) merge da pilha, na ordem; (3) auditoria independente das correções — `QA.md` mantém como REPROVOU: TR-06, TR-07, OF-08, TR-08, OF-05, AJ-03, AN-02, OF-04, TR-09, TR-10, CT-02, AN-06, AN-03, AN-04, AJ-04, VS-06; (4) B7 (fila não drena sozinha depois de logar de novo) e B8 (404 padrão do Next), que não têm teste vermelho; (5) as 5 contas QA antigas; (6) o que já estava: cápsula TREINO / TRABALHO, PE-06, P1/P2.
- **Próximo passo:** ler o CI deste PR e dos três anteriores; se tudo verde, a pilha está pronta para o dono revisar.
- **Para o outro agente saber:** dez coisas. (8) **Em produção, erro lançado numa server action chega ao cliente sem a mensagem**, só com `digest`. Lógica de cliente que decide algo por `error.message` funciona no `next dev` e falha no build (medido no run `34784135645`). A fila offline já foi corrigida (`ResultadoGravacaoSerie`, `DECISIONS.md` 2026-09-13 (2)); informação que o cliente precisa ler volta como valor. (9) **Vermelho em `j10`–`j13` é ACHADO, não teste a ajustar.** Os três defeitos de teste da 1ª rodada estão documentados no relatório; qualquer outro vermelho novo precisa da mesma triagem antes de mexer no spec. (10) **O Chrome do Playwright MCP desta máquina guarda uma credencial salva** (`qa-lastro-perf@example.com`) que não é conta QA conhecida — não usar como instrumento. (1) A `FF5` foi **emendada** — leia a seção abaixo antes de escrever consulta. (2) **A trava da prescrição não está na tela**: está em `src/app/api/analise/route.ts`, ANTES do gasto de cota. (3) **NÃO existem mais contas separadas.** A emenda 2026-09-12 (2) do PRD §11 derrubou o "duas contas" da emenda anterior: toda conta treina, e `tipo_conta = 'personal'` quer dizer "tem área de trabalho". **Os guardas leem `obterPerfil().modo`** (`treino` | `trabalho`), não `tipoConta`. (4) **A própria conta não escreve `tipo_conta` nem `cref`**: GRANT de update por coluna na 0025. Ganhar a área de trabalho é só por `ativar_area_de_trabalho()`. Coluna nova em `usuario` que o app precise escrever exige GRANT novo — sem ele o update falha. (5) **`j4` e `j5` varrem com DUAS contas**; se falharem numa tela sem relação com o módulo, olhe o `beforeAll` antes da tela. (6) **Erro de tela se lê por `.aviso-erro`, nunca `getByRole("alert")`**: o Next injeta `#__next-route-announcer__` com `role="alert"` em toda página. (7) **Sinal de "terminou" em teste tem de ser algo que não pode existir antes** (`DECISIONS.md` "2026-09-12 (1)").
