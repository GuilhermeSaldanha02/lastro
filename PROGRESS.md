...
- **Resultado da 1ª rodada (run `34784135645`):** 89 passed, 19 failed, 1 flaky. Triagem: **16 achados reais e 3 defeitos do próprio teste**, corrigidos no commit `10f51ba`. **2ª rodada (run `34785642721`, commit `10f51ba`): 90 passed, 20 failed, 0 flaky** — os 20 são achados: M3 agora medido certo, o login com e-mail gigante passou (não é achado), o M1 passou de primeira (intermitente, 1 falha em 3) e o caso novo provou a consequência do A1 (OF-08).
...
- **Em andamento:** nada. As duas rodadas do CI terminaram; o CI do PR fica vermelho por desenho enquanto os achados estiverem abertos.
- **Não commitado:** nada; relatório, `QA.md`, evidências e este bloco estão commitados e com push na branch.
- **Bloqueado / a decidir — tudo do dono:** (1) **triar os 17 achados** e decidir o que corrigir e em que ordem; nenhum foi corrigido, de propósito; (2) merge do PR #245 — com os achados abertos, o CI dele fica VERMELHO por desenho até as correções; (3) as 5 contas QA antigas no banco (as 4 da `j9` de hoje e uma da `j2` de 2026-09-05, esta última nova para a lista); (4) o que já estava: olho na cápsula TREINO / TRABALHO e nas cascas, PE-06, perguntas aos personais P1/P2.
