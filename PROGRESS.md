...
- **Última sessão:** 2026-09-13 · agente: claude · branch `fix/fila-apos-login-e-pagina-404`, **empilhada: #245 (QA) → #246 (A1/A2) → #247 (M2/M3) → #248 (M4/M5) → #249 (M1) → #250 (B1/B2) → #251 (B3/B4) → #252 (B5/B6) → esta (B7/B8)**. Sem merge; merge na ordem da pilha.
- **O que foi feito:** o dono pediu B7 e B8. (B7) `SincronizadorGlobal` drena a fila a cada troca de rota, e não só ao montar — o login navega sem remontar o layout. (B8) `src/app/not-found.tsx`: 404 com o cabeçalho do app, texto no idioma da conta e "Voltar ao início". Decisão em `DECISIONS.md` "2026-09-13 (9)". **Com isso, os 15 achados do QA do caminho triste têm correção na pilha** (M5 só reduzido; ver bloqueios).
- **Antes, na mesma sessão (CI de cada PR):** #246 `34793295556`, #247 `34795232035` e #248 `34795910058` com os casos corrigidos verdes; #249 `34797258298`, #250 `34797495882`, #251 `34797779114` e #252 `34798173235` em andamento no fechamento desta entrada.
- **Verificado localmente:** vitest 47 arquivos e 505 testes (contagem crua), ESLint e `tsc` limpos. Sem navegador local.
- **Esperado no CI desta branch:** tudo verde. O B7 não tinha teste vermelho (só anotação na `j10` "sessão: expirar no meio do treino"); a anotação "depois do novo login a fila não drenou sozinha" deve sumir do log.
- **Em andamento:** o CI deste PR e dos #249 a #252.
- **Não commitado:** nada.
- **Bloqueado / a decidir — tudo do dono:** (1) índice único parcial em `parecer (usuario_id) where status = 'gerando'` — fecha o M5 de vez; é migração, não aplicada nem escrita; (2) merge da pilha, na ordem; (3) auditoria independente das correções — `QA.md` mantém como REPROVOU todos os itens corrigidos nesta sessão; (4) as 5 contas QA antigas; (5) o que já estava: cápsula TREINO / TRABALHO, PE-06, P1/P2.
- **Próximo passo:** ler o CI dos PRs #249 a este; tudo verde = pilha pronta para o dono revisar e para a auditoria independente.
