...
- **Última sessão:** 2026-09-16 · agente: codex · branch: `codex/redesenho-tabela-treino` (PR #265 aberta).
- **O que foi feito:** Redesign estrutural de `/treino/[id]` concluído: grade contínua de séries, tipos AQ/VAL e recorde localizados, descanso realmente medido e exibido na série anterior, timer do topo ligado ao mesmo estado e persistência FIFO offline. A falha E2E da PR foi triada: os cinco cenários J10 buscavam a classe visual removida `.linha-serie-pro`; agora usam a grade atual `.grade-series__linha`, sem alterar o comportamento testado.
- **Verificado:** `npm test` com **54 arquivos / 675 testes**, `npx tsc --noEmit`, `npx eslint .`, `npm run build` e `git diff --check` passaram após a correção; `next-env.d.ts` foi restaurado. E2E local não foi executado porque usa produção.
- **Em andamento:** enviar a correção da PR #265, validar o CI rápido e a prévia Vercel; E2E passa a ser manual apenas nos marcos de integração.
- **Não commitado:** atualização deste handoff em `PROGRESS.md`.
- **Bloqueado / a decidir:** a CLI/credenciais do Supabase não estão nesta máquina, então a confirmação da migration `descanso_real_segundos` depende do E2E manual contra o banco hospedado.
- **Próximo passo:** validar preview mobile com o dono e integrar a PR #265 após o CI rápido verde.
