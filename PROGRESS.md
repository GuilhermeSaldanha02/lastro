...
- **Última sessão:** 2026-09-13 · agente: claude · branch `fix/fila-por-conta`, **empilhada: #245 (QA) → #246 (A1/A2) → #247 (M2/M3) → #248 (M4/M5) → esta (M1)**. Sem merge; merge na ordem da pilha.
- **O que foi feito:** o dono pediu M1. Cada item da fila offline grava `usuarioId` (de `obterPerfil().id`, sessão do servidor), e a sincronização só envia os itens da conta logada (`src/lib/offline/conta-da-sessao.ts`) e os antigos sem dono; item de outra conta espera por ela. Decisão em `DECISIONS.md` "2026-09-13 (5)".
- **Antes, na mesma sessão (CI de cada PR):** #246 run `34793295556` (A1/A2 e OF-08 verdes, 13 vermelhos); #247 run `34795232035` (M2/M3 verdes, `j1` e aparelho compartilhado verdes, 11 vermelhos); #248 run `34795910058` (3 casos de modelo e a concorrência da Análise verdes, 409/429 verde, 7 vermelhos).
- **Verificado localmente:** vitest 477/477 (4 casos novos em `outbox.test.ts`, 1 em `sincronizar-pendentes.test.ts`), `tsc` limpo, ESLint sem erro (o aviso de `EtiquetaRecorde` sem uso em `treino-detalhe.tsx` já existia na base). Sem navegador local; a prova de tela é o E2E.
- **Esperado no CI desta branch:** "aparelho compartilhado" da `j10` verde — mas ele era intermitente, e verde numa execução não prova sozinho. Restam os 7 vermelhos do #248 por desenho (B1 a B7 e o nome gigante).
- **Em andamento:** o CI do PR desta branch.
- **Não commitado:** nada.
- **Bloqueado / a decidir — tudo do dono:** (1) índice único parcial em `parecer (usuario_id) where status = 'gerando'` — fecha o M5 de vez; é migração, não aplicada nem escrita; (2) merge da pilha, na ordem; (3) auditoria independente das correções — `QA.md` mantém TR-06, TR-07, OF-08, TR-08, OF-05, AJ-03, AN-02 e OF-04 como REPROVOU; (4) a série B (B1 a B8); (5) as 5 contas QA antigas; (6) o que já estava: cápsula TREINO / TRABALHO, PE-06, P1/P2.
- **Próximo passo:** ler o CI deste PR.
