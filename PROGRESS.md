...
- **Última sessão:** 2026-09-13 · agente: claude · branch `fix/indice-unico-parecer-gerando`, **empilhada: #245 (QA) → #246 → #247 → #248 → #249 → #250 → #251 → #252 → #253 → esta (índice do M5)**. Sem merge; merge na ordem da pilha.
- **O que foi feito:** o dono mandou aplicar o índice único do M5. **Migração `0027_parecer_uma_geracao_por_usuario` APLICADA EM PRODUÇÃO** (índice único parcial em `parecer (usuario_id) where status = 'gerando'`), depois de conferir 0 duplicatas. O arquivo está em `supabase/migrations/`. `POST /api/analise` responde 409 para `23505` e o desempate por leitura saiu. Decisão em `DECISIONS.md` "2026-09-13 (10)".
- **Efeito imediato na `main`:** pedido simultâneo perdedor à Análise recebe 500 em vez de 202 e não cria segundo rascunho; ainda gasta a vaga de cota até a pilha chegar à `main`.
- **CI na mesma sessão:** #249 `34797258298` (7 vermelhos, M1 verde), #250 `34797495882` (5 vermelhos, B1/B2 verdes; `j1` instável por falha do Supabase ao criar conta descartável), #251 `34797779114` (4 vermelhos: B5, B6 x2 e o coach concorrente — ver achado novo); #252 e #253 em andamento.
- **Achado novo:** o coach fura o teto de 10 com pedidos simultâneos (12 usos, CI do #251, duas tentativas). Mesma corrida do M5, sem garantia no banco. `QA.md` AN-05 diz PASSOU e está errado. Não corrigido.
- **Verificado localmente:** ESLint e `tsc` limpos na rota. Índice conferido em `pg_indexes` depois de aplicar.
- **Em andamento:** CI deste PR, do #252 e do #253.
- **Não commitado:** nada.
- **Bloqueado / a decidir — tudo do dono:** (1) corrigir o coach concorrente (mesmo tipo de garantia no banco, ou contagem atômica); (2) merge da pilha, na ordem; (3) auditoria independente das correções; (4) corrigir AN-05 no `QA.md`; (5) as 5 contas QA antigas; (6) o que já estava: cápsula TREINO / TRABALHO, PE-06, P1/P2.
- **Próximo passo:** ler o CI deste PR e dos #252 e #253.
