...
> **➡️ Próximo passo: rodar o gate visual completo da seção G de [`docs/BACKLOG-PROXIMA-FASE.md`](docs/BACKLOG-PROXIMA-FASE.md).** A Leva 1 inteira (A1, B1+B2, A2+A3+A4) está implementada nesta sessão, na branch `chore/leva-1-higiene-visual`, ainda **sem PR aberto**. O gate ainda não rodou — comece por lá.
...
**Verificação rodada nesta sessão:** `tsc`/`test` (108 passando)/`lint` (0 erros)/`build`, todos verdes, do zero (`rm -rf .next`). Conferido também num navegador real (usuário QA `qa-leva1-2026-08-13@example.com`, removido ao final, cascade = 0): `/perfil` renderiza o avatar de iniciais com os tokens corretos (`getComputedStyle` confirmou `--lastro-sup-2`/`--lastro-txt`/`--lastro-controle` na base e o petróleo intacto em `.barra-topo`); `/catalogo` mostra a nota uma vez, no topo, sem `.ficha__pendente`; `/analise` no estado de espera mostra botão + 5 cards com `aria-disabled="true"`, `min-height: 48px` preservado, clique confirmado inerte (não dispara `/api/analise`), ordem primária→4 secundárias correta.

**O que este navegador NÃO conseguiu confirmar — fica para o gate G0-G5:** o painel deste ambiente não composita frames (`screenshot` falha com "the Browser pane is not displayed"), então **nenhuma medição de contraste real nem inspeção visual a olho foi feita** — só leitura de DOM/`getComputedStyle`, que `DESIGN.md` e as próprias regras do projeto tratam como não-substituto do olho. Falta especificamente: G2 (contraste medido, inclusive o valor real de `--lastro-txt` sobre `--lastro-sup-2`, que a emenda do A1 em `DESIGN.md` §3.1 deixou como "a confirmar" de propósito — não escrever um número ali sem medir); G3 (teclado/foco, incluindo K3 nas duas variantes novas dos cards); G4 (alvos de toque, ainda que `min-height` já tenha sido conferido via DOM); e G5, que só o dono fecha, no aparelho dele. Os viewports 360×640 e 390×844 também não foram olhados de verdade — só o 390 foi usado pra `getComputedStyle`, sem imagem.
