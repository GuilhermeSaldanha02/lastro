...
| **Peso por lado** (`pesoPorLado`, atributo do EXERCÍCIO) | O peso registrado é de **um** implemento (ex.: um halter em cada mão), não do par — dobra o volume, igual `unilateral`, mas por razão distinta (peso por implemento, não reps por lado). Vive no exercício (`exercicio.peso_por_lado`), não na série — a pessoa não re-declara a cada registro. **Adicionado 2026-08-24**, substitui "peso corporal incluso" (nunca usado, 0 de 461 séries — ver `DECISIONS.md`). | Sem a correção, halter bilateral (ex.: "Supino reto com halteres", `unilateral=false` corretamente) tinha o volume subestimado pela metade. |
