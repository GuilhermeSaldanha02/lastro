...
| **Peso por lado** (`pesoPorLado`, atributo da SÉRIE) | O peso registrado é de **um** implemento (ex.: um halter em cada mão), não do par — dobra o volume, igual `unilateral`, mas por razão distinta (peso por implemento, não reps por lado). Decidido **por série**, por um interruptor no formulário (`serie.peso_por_lado`) — não pelo catálogo sozinho. `exercicio.peso_por_lado` continua existindo, mas só como valor-padrão que pré-marca o interruptor para exercícios de halter conhecidos; uma lista fixa de exercícios não cobre todo uso real de halter (achado do dono testando ao vivo, 2026-08-24). Substitui "peso corporal incluso" (nunca usado, 0 de 461 séries — ver `DECISIONS.md`). | Sem a correção, halter bilateral (ex.: "Supino reto com halteres", `unilateral=false` corretamente) tinha o volume subestimado pela metade. Sem o interruptor por série, só os 9 exercícios pré-cadastrados no catálogo seriam corrigidos. |
